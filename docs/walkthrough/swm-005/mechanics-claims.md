...
- **M-21**：**整條鏈讀出來了**（`re/121`）。答 `No` 觸發的不是戰鬥而是**毒氣**：
  ...
  所以「會觸發戰鬥」對，「直接」要拆開。
- **M-14**：**後半解了**（`re/123`）。攻擊的目標篩選在 `sub_122E9`：
  同一張地圖、距離 < 接戰值 `ds:46CCh`、而且**座標在 19 × 9 的地圖視窗內**
  （`sub_169CF`）。一組都不合格就印字串表 1 第 57 條
  `No one is within range.`（`0x1237C`）。肖像框畫的是同一個佇列裡最近的一組
  （`re/115`），所以「像沒出現」與「打不到」是同一個謂詞的兩個出口——
  `RUN` 往前走正好同時解決兩者。
