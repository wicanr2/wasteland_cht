...
loc_13AC2:
0x13AC2  sub_13AE4（挑要用什麼；**尾巴會再問方向**，docs/re/132 §1）
0x13AC8  失敗 → 回頭重問
...
### 3.1 `sub_198F0` 本體：清單框架 ＋ 一份可還原的備份

```
0x198F0  ds:0CCDAh ← 0
0x198F5  sub_1997B(0)                 ; 建表 ＋ 畫 ＋ 等按鍵
0x198F8  回傳 ≠ 0FEh → 直接帶著它回去
0x198FF  ds:469Dh ＝ 1（一項都沒有）→ loc_199D1
0x19909  60 bytes：角色 +0x80…+0xBB → 暫存 ds:7931h     ; **備份**
0x1991F  push ds:7DF3h／ds:7DF5h；改成 0x34／0          ; 熱區暫時換成清單的
loc_19933:
  sub_1997B(0A8h)                     ; 再畫一次
  CF ＝ 0 → loc_19963（選到了）
  al ＝ 0FFh 且 dl ＝ 1 → 60 bytes 從暫存抄回角色記錄、還原熱區、重跑整支
loc_19963:
  sub_16F20                           ; 收畫面
  bl ← sub_19AC3(ds:4698h)            ; 這一列 → 記錄位移
  sub_199D9(bl, ds:0CCDAh)            ; **把那一格搬進暫存並在記錄裡清成 0**
  jmp loc_19933                       ; 再畫一次
```

`sub_1997B(al)` 是建表：從 `+0x80` 每 2 bytes 掃到 `+0xBE`，
**編號非 0 且 ≤ 0x24 的才算一項**（技能表只有 35 個，`docs/re/17` §3），
一項都沒有就回 `0xFF`；有就印表頭字串 `0x99`、把逐列回呼設成 `0x9582`、
`ds:470Eh ← 0x12`，然後交給清單框架的 `sub_16DB4`（畫一頁）＋
`sub_16D34`（等按鍵）——**就是 `docs/re/53` 那一套**。

`sub_199D9` 開頭的 `call $+3` 是把函式本體跑兩次的寫法（呼叫下一行，
`retn` 把那個位址彈回來再跑一次），所以它一次搬 **2 bytes** ＝ 一整格
（編號 ＋ 等級）。

推論等級：**已確認**。

### 3.2 所以「超過九項」是翻頁，不是別的機制

`sub_198F0` 自己不排版：畫面與按鍵全部走 `docs/re/53` 的清單框架。
那一套的規則是**一頁 `ds:469Eh` 列、`I`／`K`（含方向鍵）翻頁、
`1`–`9` 選這一頁的第幾列**，沒有第 10 個選擇鍵，所以第 10 項起一定要翻頁。

`I`／`K` 不是隨手挑的鍵：它們就是原版的上下方向鍵——
地圖指令列的字串裡寫著 `IKJL`（`docs/re/72` §4），
滑鼠四個楔形送出去的也是同一組（`ds:C05Dh`，`docs/re/112` §4）。
...
### 4.2 收尾：兩條路都會改寫**目標格**（方向指到的那一格，`docs/re/132`）
...
存在 `ds:A5D9h`；`sub_17CFF` 的 CF 決定走一步還是只捲動。）
