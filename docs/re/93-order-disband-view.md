...
| `DISBAND` | **多隊伍**（最多 4 組） | **已接**（`internal/play/groups.go`）|
| `VIEW` | **多隊伍** | **已接**：切到下一組有人的；繞回起點就什麼都不做 |

三支裡兩支卡在同一件事，所以**多隊伍是一個前置工項**，
不是三個獨立的指令各缺一塊——先把資料層做出來，兩支指令就各自只剩幾十行。

存檔的四組槽表各自帶座標與地圖，所以切組是「換人 ＋ 換座標 ＋ 換地圖」；
切之前要先把目前的座標寫回槽表，不然回來會站在別組的位置上。

多組同時在圖上的互動也接上了：遭遇掃描對四組各評一次
（`docs/re/39` §4），**只有同一張地圖的組參與**，而且距離用各組自己的座標。
`PartyGroupState` 因此加了 `X`／`Y`——先前拿目前這組的座標算全部，
