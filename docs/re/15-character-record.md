...
門檻表在 `ds:CCCDh`（線性 `0x29AED`），前五個 byte 是 `FF F5 EC E2 D8`。
索引是 `dl` 本身，而迴圈從 `dl = 4` 起、`dl` 減到 0 就跳出——
**所以第 0 格 `FF` 一次都沒被讀到**，實際用到的只有後四個：

| bytes | 當有號數 |
|---|---|
| `F5 EC E2 D8` | **−11、−20、−30、−40** |

比較是 `cmp al, 門檻` ＋ `cmc` ＋ `jnb` ＋ `jz` ——也就是
**「CON 低位 ≤ 門檻就回傳當下的 `dl`」**（`cmc` ＋ `jnb` 是這份執行檔的
「小於」慣用法，`jz` 補上相等那一格）。逐值展開：

| 等級 | CON | 狀態字 |
|---:|---|---|
| 5 | **恰為 0**（`+0x1D`／`+0x1E` 兩個 byte 都 0，走開頭那條捷徑）| 骷髏字模 |
| 0 | −1 … −10 | `UNC` |
| 1 | −11 … −19 | `SER` |
| 2 | −20 … −29 | `CRT` |
| 3 | −30 … −39 | `MRT` |
...
⚠ **等級 0 是昏迷，不是「沒事」。** 這一支只在角色倒下時被呼叫
（`docs/re/99` §3），CON 為正的人根本不查它——所以「0 ＝ 健康」是誤讀，
而它會讓死亡（等級 5）與昏迷（等級 0）在呈現層對調。

⚠ 比較走的是**無號**運算元。定義域是 CON ≤ 0，低位 byte 因此落在
`0x00` 與 `0xFF`–`0x80` 這一段，無號比較剛好單調——**換成有號會得到相反的順序**。
