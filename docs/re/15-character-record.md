...
**23 個位移解出、127 處存取因為位移是算出來的而追不到**——
所以下表是下界，不是全部）。

| 位移 | 大小 | 語意 | 依據 | 等級 |
|---|---|---|---|---|
| `+0x00` | 字串 | **名字** | `sub_171B9` 把 `ds:46B5h/46B6h` 直接搬進字串指標 `ds:4680h/4681h`，也就是從記錄開頭當 NUL 結尾字串印 | 已確認 |
| `+0x15`–`+0x17` | 24-bit | **金錢** | `sub_17B3E` 用 `cmp`＋兩個 `sbb` 比大小、`sub_17B15` 用 `sub`＋兩個 `sbb` 扣款，對象是 `ds:466Bh`–`466Dh` | 已確認 |
| `+0x18` | byte | 小碼，`sub_19614` 每次算完記錄就取出放 `ds:470Bh`；`sub_19362` 印它時做 `add al, 0A2h` 當訊息碼 | 強證據 |
| `+0x19` | byte | 小碼，`sub_19362` 印它時做 `add al, 91h` 當訊息碼 | 強證據 |
| `+0x1A` | byte | 名片行第 `0x11` 欄印的值 | 假說 |
...
| `+0x1F` | byte | **選中／裝備中的欄位索引**：`shl 1 ＋ 0xBC` 之後當成 `+0xBD` 那個陣列的位移 | 強證據 |
| `+0x28` | byte | 非 0 時名片行開反白 | 假說 |
| `+0x80`–`+0xBB` | 30 × 2 | **陣列 A**：`sub_198CD` 從 `0x80` 每次 +2 掃到 `0xBC` 停 | 已確認（結構） |
| `+0xBD`–`+0xF8` | 30 × 2 | **陣列 B**：`sub_1968C`／`sub_1963A` 從 `0xBD` 每次 +2 掃到 `0xF9` 停；`sub_1963A` 用 `(位移 − 0xBB) ÷ 2` 當索引 | 已確認（結構） |

兩個 30 槽陣列的內容各是什麼（物品／技能）尚未分辨，標**未解**——
兩者的掃描形狀一模一樣，不能只靠位置猜。
...
推論等級：**強證據**（程式碼讀完、兩張表從映像讀出；狀態字本身還沒對到文字）。
...
| — | 名字（`sub_171B9`） |
| `0x11` | `+0x1A` |
| `0x15` | `+0x1F` 指到的陣列 B 項目（`and al, 3Fh` 之後） |
| `0x18` | `+0x1B/+0x1C` 這個 16-bit 值 |
| `0x1C` | `+0x1D/+0x1E`；為負或為零時改印狀態字 |
| `0x20` | `sub_196C9` 取回的另一項 |
