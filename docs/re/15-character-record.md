...
| `+0x15`–`+0x17` | 24-bit | **金錢**（先比大小、不夠就失敗；商店比價與扣款走同一組，`docs/re/22` §5） | `sub_17B3E` 用 `cmp`＋兩個 `sbb` 比大小、`sub_17B15` 用 `sub`＋兩個 `sbb` 扣款，對象是 `ds:466Bh`–`466Dh` | 已確認 |
...
| `+0x21`–`+0x23` | 24-bit | **經驗值**：累計、只加不減（`sub_19BC0`）；升級門檻拿它跟 `(L² − L) × 512` 比（`docs/re/31` §1） | 已確認 |
| `+0x24` | byte | **等級**（建立時 1；升級時寫入新等級，同時 `+0x20` ＋1） | 已確認 |
| `+0x26`–`+0x27` | 16-bit | 受傷前的 CON 備份（`sub_157D6` 扣血時寫入） | 強證據 |
| `+0x28` | byte | **八個狀態位元**（`0x01` Radiation poisoning … `0x80` D7）；高四位會隨時間惡化。非 0 時名片行順便開反白（`docs/re/35`） | 已確認 |
