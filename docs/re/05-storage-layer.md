...
- **idx 7 沒有檔名，只有磁區座標**（cylinder 1、head 0、sector 1），
  而且**全檔沒有任何人用這個索引**。`sub_11445` 的 10 個呼叫端傳的
  `dl` 只有 `{0, 1, 2, 6}`：

  ```
  0x183D5 →1  0x18466 →1  0x18542 →0  0x18608 →1  0x1866A →1
  0x186CC →2  0x1874C →1  0x18784 →1  0x18815 →1  0x1B801 →6
  ```

  §2 的 `+3` 切換只把 0–2 變成 3–5（`bx < 6` 才切），**7 永遠到不了**。

  ⚠ 之前寫「這一筆很可能是存檔區」是錯的——存檔其實在 `GAME1`／`GAME2`
  **檔尾的一個 MSQ 資源**（`docs/re/30` §1），走的是一般的檔案路徑。

  合理的解釋是這張表保留了**磁片版**的欄位：`int 25h`／`26h`（絕對磁區讀寫）
  的程式碼確實還在（`0x116FA`／`0x118A3`），但 idx 0–6 的磁區座標全是 0，
  只有 idx 7 有真座標。我們手上這份是硬碟安裝版，只走檔名路徑。
  **idx 7 在這份資料裡開不起來，也沒有人開它。**
...
| `ds:9168h == 0x40` 這個磁碟切換旗標由誰設定 | **資源載入器**（`0x185F4`／`0x18656`／`0x186B8` 依資源編號設，`0x183CD` 那一族 `xor 0C0h` 換片）；存檔寫入端拿它挑要寫哪一份（`docs/re/30` §5.1）|
