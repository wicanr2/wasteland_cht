...
或狀態位元 ≠ 0」。`sub_19A1D` 的逐值對照見 [`docs/re/15`](15-character-record.md) §3：

> **等級 0 ＝ CON 在 −1 … −10 之間**（昏迷，`UNC`）。
...
|---|---|
| 有人還站著（`CON > 0`）| 什麼都不做 |
| 全倒，但有人 CON ∈ [−10, −1] **且** `+0x28` 的八個狀態位元全 0 | 自動 `View` 換隊 |
| 其餘 | 死亡畫面 |

「其餘」涵蓋 CON ≤ −11（等級 1–4）、CON 恰為 0（等級 5）、
以及雖然在昏迷帶但帶著狀態位元的人。

推論等級：**已確認**（門檻表的四個 byte 從映像讀出、比較方向逐指令對過）。
