...
0x1717B  sub_19E2A                        ; bit7 設起來時多做一件事（未解）
0x1717E  pop ax
loc_1717F:
0x1717F  ds:4687h ← 1
...
**未解**：附屬 byte 的 bit7 設起來時 `sub_19E2A` 做什麼。
它在 `AMM` 那一欄是「印 0」的訊號，在這裡多做一件事——
兩處都指向「這件武器現在不能用」，但**沒有直讀**，所以不寫進規格。

推論等級：**已確認**（欄位來源與印出的內容）／**未解**（`sub_19E2A`）。
