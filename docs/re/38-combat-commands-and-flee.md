...
- **隊形**。`ds:46CCh`／`ds:46CDh` 已經解了，是「接戰值」與「武器類別」，
  和站位無關（[`45`](45-item-data-and-weapon-damage.md) §4.1）；
  仍然沒有讀到「站位」這種東西存在的證據——不要先假設有。
- 跳表索引 9（`0x14820`）是誰用的。
- `sub_190B8`。

指令 1（Hire）／3（Weapon）／6（Load）／7（Use）四支處理程式已解，見 `docs/re/41`。
