...
> **是 XOR 進螢幕，不是疊回圖片緩衝區。** 這一點決定了所有實驗的成敗：
> 在解碼後的 96 × 84 packed 4bpp 上怎麼疊都對不上，因為原版根本沒碰那份緩衝區。

驗算：第 3 張圖的 88 個元素照這個解法算出來是 **x 56–79、y 8–18**，
而且每個元素的酬載長度都等於「長度 ＋ 1」——與 `docs/re/54` §3 那 126 個
對不上的像素**同一塊**。

推論等級：**已確認**（`sub_10B11` 逐條讀完；解法自洽；範圍與實機殘差一致）。
