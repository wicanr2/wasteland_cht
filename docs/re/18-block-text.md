...
全部解出來是 **4,493 條非空字串**，最長 696 個字元。內容是遊戲的敘述文字：
地點進出訊息、物件描述、NPC 對話、劇情段落、製作名單。

加上執行檔的 442 條，原版的文字總量是 **4,935 條**。
...
- **這 4,493 條是中文化的主體**，比段落書（162 段）大一個數量級。
...
- **字串索引怎麼對到地圖上的位置**：記錄本體（`ds:46AEh`）的 `+0x03` 有 34 處
  ...
  （`docs/re/16` §4）。
- 段落書 162 段與這 4,493 條的關係：遊戲內既然有敘述，
