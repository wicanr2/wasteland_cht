...
| 欄位 | 路徑 | 函式 |
|---|---|---|
| `+0x15`–`+0x17` | 減（先比大小再借位扣） | `sub_17B3E` ＋ `sub_17B15` |
| `+0x21`–`+0x23` | 加（進位鏈） | `sub_19BC0` |

兩個都是 24-bit，但**是不同欄位**，而且只各有一個方向的路徑。
`+0x15` 因為「先比大小、不夠就失敗」的形狀，語意像可花用的資源（`docs/re/15`）；
`+0x21` 只加不減。**兩者各是什麼未解**，不要憑「一個是錢一個是經驗」猜。

## 7. 還沒解的

- **命中判定**：`sub_157D6` 收到的是已經算好的傷害，計算它的地方還沒讀
  （`sub_141FA` 的另一個呼叫端 `0x1444F`、以及 `0x1B0C6` 那條路徑）。
- `+0x21`–`+0x23` 與 `+0x15`–`+0x17` 各代表什麼。
