...
| `0x7DF3`／`0x7DF5` | **這個畫面有哪些熱區／框邊標籤**（32-bit 遮罩，bit n ＝ 熱區 n）|
| `0x8DD2`／`0x8DD4` | 同上，但「現在畫著的是哪些」——與想要的取 XOR ＝ 要畫或要擦的 |
| `0x471F` | **這一行的序號要不要反白**（「現在輪到／選中的是這個人」）|
...
| `0x46EE` | **輻射讀數**：視野內最近 nibble 9 格的距離；`0FFh` ＝ 沒有讀數（視野裡沒有／隊上沒有蓋氏計數器）|
...
| `0x13878` | 多 | **算接戰值** `ds:46CCh`：0 不能行動／`0x0F` 近戰／`0xFE` 有射程武器且彈匣非空 | [`123`](123-attack-target-range.md) §2 |
| `0x122E9` | 2 | **這一組列不列得出來**：同地圖 ＋ 距離 < `ds:46CCh` ＋ 在 19 × 9 視窗內 | [`123`](123-attack-target-range.md) §1 |
| `0x17E42` | 4 | **畫輻射計量表**（字元欄 38–39）：先找全隊有沒有物品 48，沒有就把讀數蓋成 `0FFh` | [`120`](120-geiger-counter.md) §1、§3 |
| `0x17DF1` | 多 | 畫時鐘（欄 28、列 0），**接著就落進 `0x17E42`** | [`27`](27-game-clock.md) §3、[`120`](120-geiger-counter.md) §3 |
...
| `0x17029`／`0x1702E`／`0x17033` | 6／2／11 | **重畫一列名片行**的三個進入點：序號反白開／關／沿用（`ds:471Fh`），本體從 `0x17037` 起 | [`128`](128-roster-index-inverse.md) §1 |
| `0x18A4F` | 2 | 逐位比對熱區遮罩，差異的才畫（`sub_18B6B`）或擦（`sub_18B3C`） | [`129`](129-label-masks.md) §2 |
| `0x18B6B` | 1 | 畫一個框邊標籤：版面表第 (bl−4) 筆；**bl < 8 走直式** | [`129`](129-label-masks.md) §3 |
| `0x19130` | 3 | 按數字選一個人之後的小迴圈：序號反白、`P` 集中金錢、`D` 平分、Enter／空白離開 | [`128`](128-roster-index-inverse.md) §4 |
...
| [`116`](116-enemy-move-execution.md) | 敵人在地圖上移動的落地：九步向表 `ds:AAB1h`、距離表 `ds:CD0Dh`、射程帶 `sub_156DE`、分數 ＝ 起始 ＋ 增量 × ⌈距離/3⌉，閘門是「行動值 ＋ 種類常數 < 分數 → 換位置」|
| [`117`](117-save-globals-and-facility-screen.md) | 存檔全域狀態 14 bytes（`ds:464Eh`–`465Bh`）：**`ds:4655h` ＝ 目前地圖**（讀檔不看隊伍槽表）；設施畫面 ＝ 名單模式（肖像框 ＋ 選單區 ＋ 名單 ＋ 指令列）|
| [`118`](118-shop-stock-groups.md) | 物品表有四份（`ds:BE20h` ＋ {0, 0x2FE, 0x5FC} 與 `game2` 一份），**只有庫存 `+0x02` 不同**；商店按記錄 `+0x06` 選組 |
...
| [`127`](127-roster-mode-boxes.md) | 名單模式的兩圈框：肖像框 (0,0)–(13,13)（`sub_19770`）、選單框 (14,0)–(39,13)（`sub_19727`），文字區欄 15–38、**列 1–12**。下緣標籤戰鬥是 `MAP`、設施是 `POOL MONEY`；**名單模式沒有時鐘** |
| [`128`](128-roster-index-inverse.md) | `ds:471Fh` ＝ 行首序號的反白旗標：`sub_17029`／`sub_1702E`／`sub_17033` 三個進入點共用一個名片行本體；反白只蓋序號與 `>`，意思是「現在輪到／選中的是這個人」（戰鬥指令階段、`USE`、商店／醫生／訓練師櫃檯）|
| [`129`](129-label-masks.md) | `ds:7DF3h`／`ds:7DF5h` ＝ 32-bit 熱區遮罩（bit n ＝ 熱區 n、標籤 k ＝ 熱區 k+4）：每個畫面進迴圈前宣告一次，`sub_18A4F` 只畫差異。版面表前四筆（箭頭）走**直式**，長度 2 ＝ 兩列高 |
| [`130`](130-chest-loot-pickup.md) | 寶箱撿拾（`0x1534B`–`0x15453`）：名單畫面問「誰要撿」→ 清單逐件拿（附屬 byte ＝ 容量）→ 全拿完 `sub_169B1(0)` 改寫這一格；`0x5E` ＝ 現金 3 bytes；清單的 `0x00` 是空格**續掃** |
| [`131`](131-character-screen.md) | 角色畫面（地圖 `1`–`7` → `sub_12760` → `sub_19130`）：三頁循環；裝備 `sub_1949E`（護甲 → `+0x25`＋AC）、裝填 `sub_196DB`、卸卡彈 `sub_19ACD`（IQ＋技能×3＋2d6續擲 ≥ 5×計數＋15）、交給 `0x19299`（拒絕檢定看收件人魅力對給的人 `+0x2E`）|
| [`132`](132-use-direction.md) | 地圖 `USE` 也問方向（`sub_125BD`）：目標 ＝ 方向那一格（`sub_164E0`）、方向存 `ds:A5D9h`、`sub_17CFF` 的 CF ＝ 真的改寫 → `sub_1651A` 走一步 |
| [`133`](133-medic-doctor-and-npc-flag.md) | Medic／Doctor 急救（`loc_13D82`）：只救 CON < 0，難度 ＝ |CON| >> 2／3，成功負血折半；**`+0x29` ＝ NPC 旗標**（PC ＝ 0，推翻「槽有人」）；PC 上限 4（`sub_1C6C9` 數 +0x29＝0 的人）|
| [`134`](134-item-reorder.md) | 物品重排：清單附加鍵 `0x12`（Ctrl+R）、挑走順位 ＝ 新位置、`+0x1F`／`+0x25` 跟著搬；物品那條 ESC 不能中斷、技能那條可還原 |
