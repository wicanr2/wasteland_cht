...
### 4.7b 攻擊資料表（記錄區標頭 `+0x04` 指到，8 bytes 一筆）

**和物品表不同的另一張表**——兩張共用同一支定址器（`sub_16F4C`），
差別在誰設基址（[`32`](32-skill-checks-and-xp.md) §1）。

| 位移 | 內容 |
|---|---|
| `+0x00`–`+0x01` | 經驗值基值（16-bit） |
| `+0x03` | 傷害骰數 N |
| `+0x04` 低 4 位 | 經驗值倍數 −1 |
| `+0x05` 高 4 位 | 傷害基底 |
...
| 路徑 | 位址 | 基礎值 | 命中條件 |
|---|---|---|---|
| 隊伍打敵方 | `0x1AF52` | 50／60 | `roll(1..100) < 累加值` |
| 敵方打隊伍 | `0x1B04C` | 40／50／60 | `roll(1..100) ≥ 累加值` |

累加值 ＝ 基礎 ＋ 隊伍成員技能 × 3 ＋ 欄位 ± 距離修正，夾在 100。
...
| `0xA4E0` | `0x27300` | nibble 6 的第二層跳表 A（5 項，服務型地點） |
| `0xA4EA` | `0x2730A` | nibble 6 的第二層跳表 B（44 項，末尾 word ＝ 0） |
...
| `0x46C8` | 敵方記錄的位址（`0x6B31 + 0x178a + 0x5Eb`）；開頭是 16-bit HP 陣列 |
...
完整索引在 [`00-function-index.md`](00-function-index.md)（641 個，已分析 282）。
...
| `0x18024` | 5 | 畫一格：依第 1 層 nibble 選圖形，再叫 slot 3／4 | [`25`](25-screen-layout.md) §2.1 |
...
| `0x12440` | 1 | 每 16 刻的角色狀態處理（隨時間恢復／惡化） | [`27`](27-game-clock.md) §6 |
