...
| `0x19F12` | 1 | **沖出目前這一行**：斷字 → 寫 scrollback（`seg003:0x8CE0`，40 × 256 環形）→ 送畫面 → 續行 | [`60`](60-teleport-and-map-change.md) | nibble 10 ＝ 傳送並換地圖；記錄 `+0x03` ＝ 目標地圖（`0xFF` ＝ 回程）；槽表 `+0x0B`–`+0x0D` ＝ 回程 |
