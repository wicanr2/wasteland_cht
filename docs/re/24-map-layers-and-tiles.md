...
`al` 那個小常數則是疊在上面的東西，範圍 0–9，與 `IC0_9.WLF` 和 `MASKS.WLF`
各 10 筆一致——**兩個素材的筆數同時吻合**。十個編號各自是什麼見
[`48`](48-map-icons.md)（隊伍、五種敵人、寶箱、輻射區、其他分隊、塗黑）。

`MASKS.WLF` ＝ 320 bytes ＝ **10 張 × 32**，一張是 16 列 × 2 bytes、
**一個位元一個像素**（不是 4 平面）。
