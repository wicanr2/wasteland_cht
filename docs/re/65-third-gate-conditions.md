...
| 項目 | 做法 |
|---|---|
| 逐個隊員試 | `Party.EvalParty`——`ds:A5D3h` 從 1 數到 `ds:4653h`，任一人過就算過；不能行動的（`CanCommand`）跳過 |
| 技能資料表 | `Rom.SkillTableRaw()` 讀 `ds:BA20h` 的 36 × 2，`game.SkillBytes` 實作 `SkillTable` |
| 判定的時機 | **在 `Step` 裡做一次**，不在 `passable` —— 判定會擲骰、會消耗鑰匙（`sub_19A58`） |
| 失敗的訊息 | 記錄 `+0x01` |

⚠ **`Passable`（尋路用）對條件格一律回可走。** 那是刻意的：
唯讀查詢不能有副作用，否則光是尋路就會把鑰匙用掉。
代價是尋路算出來的路可能實際走不通——那與原版玩家的處境相同。

還沒做的：通過之後會不會改寫地圖格（`sub_13EC9` 後半的 `sub_14193` 那一段）。
