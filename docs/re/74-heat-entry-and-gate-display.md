...
## 1. `sub_142ED`：暫時換掉時鐘的「時」再顯示

```
sub_142ED:
  sub_19720                    ; 讀 ds:4707h，非 0 就走另一條
  dl ← ds:A5C5h
  sub_14314(dl)                ; ds:A5E7h ← ds:46F7h；ds:A5E6h ← ds:465Bh
                               ; ds:46F7h ← 1；ds:465Bh ← dl     ← **換掉**
...
`ds:465Bh` 是時鐘的「時」，`ds:46F7h` 是跟著它的一個旗標。
`sub_18DB4(dl)` 是把 `sub_18DBE(0xFF)` 跑 dl 次的**延遲迴圈**，
所以這一段是「**換一個時間值 → 印訊息 → 停一下 → 換回來**」。

`ds:A5C5h` 由 `sub_14193`（套用懲罰）在 `0x141F4` 寫入，
`0x141D6` 與 `0x142F0` 讀——三處全在懲罰這條路上。
所以換進去的不是「另一個時刻」，而是**懲罰算出來的量**；
換掉時鐘的時是為了讓顯示層拿得到那個數字（推論等級：**假說**，
`sub_1790B` 的取值端還沒讀）。

remake 這一側只印訊息，**沒有做延遲與時鐘暫換**——
呈現層的節奏由 remake 自己決定，這一段標為刻意不移植。
