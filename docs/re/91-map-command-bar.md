...
⚠ **`+0x4B`／`+0x4C` 的語意未解**：兩個 byte 都只用到 bit0，
形狀是「條件旗標 ＋ 已處理旗標」，但條件從哪裡被設起來還沒追。
`docs/re/15` 的角色記錄表沒有這兩格。

推論等級：**已確認**（指令流與跳轉目標都讀到）／
**未解**（`+0x4B`／`+0x4C` 代表什麼）。
...
| `Radio` | **已接（第二輪）** | 逐人檢查經驗值並升級，可連升；音效 4 照播 |
| `Use`／`Enc`／`Order`／`Disband`／`View` | 未接 | 入口已定位，見 §1 的表 |

Radio 的**第一輪沒有做**：`+0x4B`／`+0x4C` 的語意未解，
照 `CLAUDE.md` §0 的規矩不猜——寧可少做一段，不要發明遊戲規則。
