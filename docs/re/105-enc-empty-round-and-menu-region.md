...
左邊那一塊畫的是誰在 [`115`](115-portrait-box.md) 解完了：**離隊伍最近的那一組敵人**
（圖 ＝ 敵人資料 `+0x07`、說明 ＝ 那一組的名字），一組都挑不到才畫遊俠
（圖 8、字串 96 `Ranger`）。這張截圖是空地遭遇，走的正是後備那一支。
...
| 指令選單畫在面板上、一行一個選項 | **已接**（`Scene.msgRect` ＋ `render.PanelCol`…）|
| 名單排得下整隊 | **已接**（`Scene.rosterLastRow`）|
| 戰鬥時指令列照留 | **已接** |
| 肖像框 | **已接**（`internal/play/portrait.go`，`docs/re/115`）|

接的時候順帶對出三個原本就錯的地方（都是實機截圖看出來的）：

| 原本 | 正確 | 症狀為什麼看不出來 |
|---|---|---|
| 名單一行 38 欄 | **39 欄** | `Crowbar` 被切成 `Crowba`，看起來只像「名字比較長」|
| 行首沒有東西 | **序號 ＋ `>`**（`0x1709A` 印數字、`0x170A0` 印 `0x3E`）| 整行往左差兩格，但每一欄自己還是對齊的 |
| 名單排到列 17 為止 | **排到列 23**（戰鬥時訊息視窗那 6 列是空的）| **第四個隊員直接不見**，看起來像「隊伍只有三個人」|
