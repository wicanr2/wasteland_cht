...
- `loc_1382B`：算「可雇用的對象數」的那一段（本節開頭那一列）。

已解的（原本列在這裡）：三條分派見 [`92`](92-use-command.md) §4；
**`sub_12738` ＝ `al << 4`**，合成的是那個 byte 的**高**半部（低半部是方向）；
**`ds:A9FDh` 沒有欄位配置**，整個 byte 就是編號——分成兩半的是指令參數
`ds:46DAh`。兩條都在 [`108`](108-combat-use-and-hire.md) §1。
