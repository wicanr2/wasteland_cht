...
## 7. 對 remake 的意思：基礎值由計畫算，不是常數

計畫本身放進 `game.Battle`（`MovePlan`，`NoMovePlan` ＝ `-1`），
命中基礎值由 `game.HitBase(plan)` 算——**不是寫死的常數**。
`internal/play/round.go` 曾經寫死 60，理由是「表未解，先用『否則』那一支」；
表解出來之後那個理由不成立了。

§6 的三條分支與 §4 的編碼在 [`116`](116-enemy-move-execution.md) 接進 remake
（`game.PlanEnemyMove` 每回合算一次，`internal/play/enemymove.go` 執行），
所以兩條都會走到：站著不動的敵人 50、這一回合要移動的 60。

⚠ 這是一個玩得出來的差異：站著不動的敵人比較難打，命中率差 10 個百分點。
