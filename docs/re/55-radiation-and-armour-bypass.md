...
0x14423  call sub_196C4
0x14426  al ← bl；bl ＝ 0 → 跳過下一步
0x1442C  al ← 角色記錄[bl]；＝ 0x29 → 跳過這個人
...
- `0x14432` 的 `cmp al, 29h`：第二輪起會拿角色記錄 `+0x01` 比 `0x29` 決定跳過誰。
  `+0x01` 在名字字串裡，這個比較的用意未解。
- `sub_196C4`（`mov bl, 25h; jmp loc_196CB`）與 `sub_169B1(2)`。
