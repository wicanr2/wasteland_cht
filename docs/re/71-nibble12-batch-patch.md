...
### 5.5 跳表索引 3 是 Ranger Center 的角色管理

`Ranger Ctr.`（資源 0 的 section 6 記錄 0，在 (55,62)）的 `+0x00` ＝ `0x83`
→ 跳表索引 **3** ＝ `0x1A2C0`，那支的選單字串是 `CREATE DELETE PLAY`
（`ds:CE42h` → `ds:CE12h`）——**建立／刪除角色與開始遊戲**，
不是通用的店面框架。詳見 `docs/re/72` §3。

`Ranger Ctr.` 的格子初始就是 nibble 6，而**旁邊的 nibble 12 會把它改寫成
傳送格**（`docs/re/72` §2）。所以「幾筆設施有格子指到」這個統計對這種系統
沒有意義——兩邊都在動。
