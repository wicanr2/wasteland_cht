...
- `sub_13762`／`sub_13C58`（遭遇相關，改寫來源是敵方記錄的前兩個 byte）。
- **`SectionCount` 是不是低估了 section 6 的長度**：資源 36 的記錄 42、
  資源 35 的記錄 192 都指到區塊外，而算法（`docs/re/16` §3.1）是
  「第一個非空指標指到哪，陣列就到哪為止」——那是推論不是讀出來的。
  如果陣列更長，設施記錄的編號可能比現在列舉的多。
- **實機**：走進一家商店，用 remake 載入同一張地圖同一格，比對 (nibble, 記錄)。
