...
這正好對上盤點 **A9**：`ALLPICS` 裡交錯著一段變動長度的參數區
（`sub_10A7A` 拆成兩張指標表，`docs/re/23` §5，**未解**）。
那段參數很可能就是這種**局部動畫**的資料——截圖抓到的是動畫的某一格，
與底圖不同。

推論等級：位置與尺寸 **已確認**（實機逐像素）；
「差異來自 A9 的動畫參數」**強證據**（位置吻合、範圍侷限、與 A9 的描述一致）。
