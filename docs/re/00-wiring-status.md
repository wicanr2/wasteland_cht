...
| 110 | [`Hire` 的結算——把 NPC 的記錄整筆複製進隊伍](110-hire-resolution.md) | 未接 | 結算流程解完了（section 17 取記錄 → 抄 256 bytes → 魅力對決 → 槽表補位）。remake 還沒實作：缺 section 17 的讀取、地圖記錄 `+0x09` 的兩個欄位、七人上限與可變長度骰。按 `H` 仍印 `ui:combat.notyet` |
