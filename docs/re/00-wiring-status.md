...
| 114 | [遭遇記錄 `+0x09` 是「這一組跟你什麼關係」](114-friendly-encounters.md) | 已接 | `internal/game/hire.go`（`Friendly`／`TurnHostile`／`friendlyBit`）、`internal/play/round.go`（`enemyActs` 跳過友善那一組、`partyActs` 開槍就翻臉）、`internal/play/hiredata_test.go`（出貨資料裡 14 筆的正面事實 ＋ JACKIE 那一場的端到端）|
