...
| 05 | [雙模式儲存層 —— 磁片絕對磁區 vs 硬碟 DOS 檔案](05-storage-layer.md) | 不適用 | 磁片版遺留的考證（資源 idx 7 沒有消費端），沒有可實作的結論 |
...
| 79 | [設施覆蓋率盤點 —— 跳表索引 ≥ 5 就是 opcode](79-facility-coverage.md) | 已接 | `internal/game/script.go`、`internal/game/world.go`（共 5 處） |
