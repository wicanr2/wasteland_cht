...
- **敵人的 miss 訊息沒有名字**：`enemyActs` 回 `"misses."`，
  原版有敵人名稱（`EnemyKind.MessageID()` ＝ 字串 `0x52 + Kind`），
  但 `CombatScene` 手上沒有字串表。
- `partyActs` 的命中累加值用「已裝備槽的物品資料」＋ 距離 0，
  射程與距離懲罰還沒接（`internal/play/round.go` 的註解已標「暫代」）。
- 敵人的目標選擇是「還能行動的人裡第一個」，原版走 `sub_15036` 的目標表
  （`docs/re/51` §8 只讀到頂層形狀）。
