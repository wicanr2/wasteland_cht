...
```
0x17094  ds:4678h ← ds:471Fh          ; 序號的反白由呼叫端決定（docs/re/128）
0x1709A  印序號與 `>`
...
三處反白共用同一個旗標，但意思分成兩族：`MAX` 與武器名那兩處是
**「這一格有問題」**，序號那一處是**「現在輪到／選中的是這個人」**
（[`128`](128-roster-index-inverse.md)）。

| 條件 | 反白的是 |
|---|---|
| 角色記錄 `+0x28`（八個狀態位元）非 0 | **`MAX`（體力上限）那一欄**——開在 `0x170F6`、關在 `0x1711F`，中間只印了 MAXCON |
| 裝備武器的附屬 byte `bit7` | **武器名字** |
| `ds:471Fh` 非 0 | **行首的序號與 `>`**——開在 `0x17094`、關在 `0x170A5`，中間只印了序號。意思是「現在輪到／選中的是這個人」（[`128`](128-roster-index-inverse.md)）|
...
- **同一個旗標被多處設起來時，語意要由「誰寫它」決定。** 這一支裡的兩處
  （狀態位元、卡彈）共用「這一格有問題」，而序號那一處來自另一個旗標
  `ds:471Fh`，講的是完全不同的事——形狀相同不代表語意相同。
