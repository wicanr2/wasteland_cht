...
也就是 **checksum ＝ 0 − Σ 明文位元組**。解密用的 key 是
`lo(checksum) ^ hi(checksum)`，所以**改寫內容之後一定要重算 checksum 再加密**，
否則原版讀進來會驗不過。實測：兩個存檔資源各驗一次都吻合（`docs/re/30` §2.1）。
