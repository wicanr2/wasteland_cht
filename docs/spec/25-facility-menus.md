# 25：設施的選單互動

狀態：**READY** ｜ 日期：2026-08-15 ｜ 對應 `internal/play`

規格 18 是設施的**規則**（庫存、賣、逐點治療、解毒），規格 23 是**場景**
（進得去、看得到、離得開）。這一份補中間的**互動**：按哪個鍵、走到哪一步。

---

## 1. 依據

| 規格內容 | 來源 | 推論等級 |
|---|---|---|
| 商店主迴圈的按鍵與字串 | [`re/42`](../re/42-facility-loops.md) §1 | 已確認 |
| 買／賣的入口條件 | [`re/42`](../re/42-facility-loops.md) §2、§3 | 已確認 |
| 每一列怎麼畫、裝備中要標記 | [`re/42`](../re/42-facility-loops.md) §3.1 | 已確認 |
| 醫生的逐點治療迴圈 | [`re/42`](../re/42-facility-loops.md) §5 | 已確認 |
| 解毒的入口 | [`re/42`](../re/42-facility-loops.md) §6 | 已確認 |

## 2. 商店

```
進場 → 招呼字串（地圖記錄 +0x05）
      隊伍只有一人 → 直接用他
      否則「Who wants to enter?」選人
      選到的人不能行動 → 「… can't buy anything.」，重選
主迴圈：「Do you want to:  Buy / Sell」＋「You have $<餘額>」
    ESC → 離開
    B   → 買：物品陣列滿了就「Your inventory is full.」回主迴圈
    S   → 賣：身上沒有賣得掉的就「You don't have anything they want!」回主迴圈
    P   → 換下一個人
    其他 → 繼續等
```

⚠ **兩個「回主迴圈」不是錯誤分支**，是原版的正常路徑——
按了 B 但背包滿了，畫面回到 Buy／Sell 而不是離開商店。

## 3. 醫生（兩層）

**主選單與治療迴圈是兩層**，這件事從字串表 8 的第 11／12／13 條看得出來——
`Exam $` ／ `Healing` ／ `Curing` 各帶一個 `\x10` 熱鍵標記，所以熱鍵是
**E／H／C**；進了 Healing 之後才是第 16 條的「Heal 1 point ／ Continue」。

```
主選單：「Exam $n / Healing / Curing」
    E → 檢查（規格 09）
    H → 進治療迴圈
    C → 進解毒選單；**沒有病就印「You have no diseases.」留在主選單**
    P → 換人   ESC → 離開

治療迴圈：「還差 N points. You can:  Heal 1 point / Continue」
    H → 治一點：扣「每點價格」→ CON +1 → **重算差額**
    C → 回主選單

解毒選單：「Which to cure at $n:」＋ 身上的病
    1–9 → 治一種；治完最後一種自動回主選單
```

⚠ **兩層寫成一層的話「C」會在錯的地方被吃掉**——主選單的 C 是 Curing、
治療迴圈的 C 是 Continue，同一個字母兩個意思。

⚠ **一次按鍵只治一點、只扣一次錢，而且每一輪重算差額**（`re/42` §5）。
差額算在迴圈外會變成「錢不夠就一點都不治」，而原版是治到沒錢就停在那裡、
已治的點數留著。

## 3.1 訓練師

依據 [`re/52`](../re/52-trainer-facility.md)。

```
進場 → 招呼字串 → 「要教誰？」
選人：
    不能行動（CON ≤ 0） → 回頭重選
    技能點數 ＝ 0        → 回頭重選
    沒有可學的           → 回頭重選
    選一個技能 → 費用 > 技能點 → 回頭
                 否則學起來（規則在規格 09，這裡不重做）
ESC → 離開
```

⚠ **三條「走不通」都是回到選人，不是離開設施**——與商店的「背包滿了」
同一種形狀。寫成錯誤分支會讓玩家莫名其妙被踢出去。

## 4. Go 介面

```go
// internal/play

// FacilityStep 是設施選單的一個狀態。
type FacilityStep int

const (
    StepMain FacilityStep = iota // 主選單
    StepBuy                      // 買的清單
    StepSell                     // 賣的清單
    StepPick                     // 選人
)

// Key 送一個按鍵給設施選單。回傳 false 表示要離開設施。
func (f *FacilityScene) Key(k byte, p *game.Party, items game.ItemTable) bool
```

## 5. 未解與邊界

- 每列回呼的參數怎麼傳還沒讀（`re/53` §5），所以清單用重製版自己的排版；
  **翻頁與選擇的語意照原版**（`re/53` §3、§4）。
- 清單一頁 9 件、**I 上一頁／K 下一頁**（`re/53` §3、§4）。
  9 這個數字的理由是**選擇鍵只有 `'1'`–`'9'`**——原版的 `ds:469Eh`
  全檔只有醫生的疾病表設過，不是清單框架的常數。

## 6. 驗收條件

1. **背包滿了按 B**：回主迴圈，不離開商店，不當成錯誤。
2. **身上沒有賣得掉的東西按 S**：同上。
3. **賣一件**：槽清成 0、錢增加、店家庫存 +1（`0xFF` 不變）。
4. **治療**：錢只夠治 2 點時，CON 只增加 2，而且**已治的點數留著**。
5. **不能行動的人**（CON ≤ 0）不能被選進商店。
6. ESC 在任何一層都回得去：清單 → 主迴圈 → 離開設施。
7. **訓練師**：技能點數 0 的人選不進去；費用大於點數時不扣點也不升級，
   而且**留在選單裡**。
8. **醫生的兩層**：主選單按 C 進解毒（沒病就留在主選單），治療迴圈按 C 回主選單。
9. **解毒**：治好一種病就從狀態位元清掉並扣錢；治完最後一種自動回主選單。
10. **分頁**：12 件的清單按 K 到第二頁，第二頁的「1」是第 10 件；到底之後
    再按 K 不動；I 回得去。
11. **列 ≠ 索引**：被濾掉的品項不佔列號，選第 1 列拿到的是「列出來的第一件」
    而不是資料的第 1 筆。
