# 11：翻譯目錄

狀態：**READY** ｜ 日期：2026-08-15
對應 `translations/`、`tools/extract_strings.py`、`tools/build_lang.py`、`internal/lang`

---

## 1. 依據

| 規格內容 | 來源 | 推論等級 |
|---|---|---|
| 執行檔九張表 444 槽／426 條非空 | [`docs/re/17`](../re/17-packed-text.md) | 已確認 |
| 42 個地圖區塊 4,445 槽／4,401 條非空 | [`docs/re/18`](../re/18-block-text.md) | 已確認 |
| 文字控制碼與變形機制 | [`docs/re/14`](../re/14-fonts-and-text-encoding.md)、[`28`](../re/28-text-variants.md) | 已確認 |
| 中文排版格與字寬 | [`docs/spec/10`](10-cjk-layout.md) | — |
| 段落編號是純文字 | [`docs/re/33`](../re/33-paragraph-references.md) | 已確認 |

## 2. 分層：原文、翻譯、編譯產物

```
translations/source/*.tsv     原文（抽出來的，機器產生，不手改）
translations/zh-Hant/*.tsv    中文（人翻的，唯一要手動維護的東西）
translations/glossary.md      譯名表（唯一真相）
translations/zh-Hant.cat      編譯產物（Big5，Go 讀這個）
```

- **原文檔是機器產生的**：`tools/extract_strings.py` 從原版資料抽，任何時候可以重跑。
  重跑之後原文有變 ＝ 抽取工具改了，要回頭看，不是手動改原文。
- ⚠ **`translations/source/` 不入版控**：那是原版的英文文本，屬於原版資料
  （`CLAUDE.md` §7 不散布）。每個人從自己那份原版抽。
  公開的只有 `zh-Hant/`（我們翻的）與 `glossary.md`。
- **中文檔是人翻的**，UTF-8、TSV、一行一條，diff 得動。
- **`.cat` 是編譯產物**：`tools/build_lang.py` 檢查排版與 Big5 覆蓋，
  存成 **UTF-8**（v2），Go 只讀它。
  **整條管線是 UTF-8，只有查倚天字模那一刻才轉 Big5**
  （`render.DrawRune`，全專案唯一一處）——倚天字型是 Big5 排列，
  被綁死的只有字型索引那一層。取捨與量測在 `docs/utf8-experiment.md`。

## 3. Key 的形狀

Key 要**穩定**（原版資料不變就不會變）而且**看得出來源**：

```
exe:<表編號>:<槽>          執行檔的九張表，表編號 0–8
blk:<檔名>:<資源編號>:<槽>  地圖區塊，例 blk:game1:12:37
ui:<名稱>                  重製版自己的介面文字
place:<英文原名>            店家招牌與地點名
monster:<英文原名>          地圖的明文敵人名字表
```

槽編號就是原版的字串索引，**不是流水號**——這樣新增／刪除都不會讓別的 key 位移。

⚠ **`place:` 與 `monster:` 用原文當 key**，因為那些字**不在字串表裡**：
招牌是地圖記錄與存檔裡的明文 ASCII（`docs/re/29` §5.4、`docs/re/30`），
敵人名字在記錄區標頭 `+0x02` 指到的明文名字表（`docs/re/114` §6）。
抽字串的工具看不到它們，所以走「顯示時拿原文查表」。

⚠ **key 裡的控制碼寫成 `\xNN`**（敵人名字用 `\x0A` 分單複數）。
`build_lang.py` 的 `read_tsv` **只 unescape 譯文，key 原樣保留**，
所以 Go 那一側查表前也要跳脫（`lang.MonsterKey`）。兩邊不一致的症狀是
**每一條有單複數的名字都查不到**，而畫面上只是顯示英文。

## 4. TSV 的欄位

```
key <TAB> text
```

- `text` 裡的控制碼寫成 `\xNN`（`\r`、`\x06`、`\x10` 這些在原版是有意義的，
  **不能刪掉也不能自己加**）。
- 空字串的槽**不進檔案**（4,889 槽裡有 62 個是空的）。
- 註解行以 `#` 開頭。

## 4b. 兩層翻譯：逐 key 與文本層

42 個地圖區塊**各自帶一份**戰鬥動詞與開門訊息，所以同樣的英文在 **1,571 個 key**
裡重複出現（4,401 條區塊字串去重之後只有 3,015 條）。逐 key 翻只會讓同一句話
被翻出好幾種版本，而且量白白多三分之一。

| 層 | 檔案 | 鍵 | 優先 |
|---|---|---|---|
| 逐 key | `translations/zh-Hant/*.tsv` | 字串 key | **高** |
| 文本層 | `translations/zh-Hant/_shared.tsv` | 原文本身 | 低 |

`build_lang.py` 先套逐 key 的，再用文本層補剩下的。**底線開頭的檔案不當成逐 key 檔**。

### 打字問答的答案必須維持原文

`translations/must-not-translate.tsv`（由 `tools/summarize_questions.py` 產生）
列出 126 條密語答案。`build_lang.py` 比對譯文與原文，**不同就整個建置失敗**。

比對是逐 byte 全等而輸入層是 ASCII（`docs/re/46` §6），翻成中文玩家永遠打不出來。
⚠ **這種錯不會有任何徵兆**——遊戲照跑、畫面照顯示，只有那道關卡過不了。
所以清單檔不存在時直接失敗，不靜靜跳過。
同一句話在某個區塊真的需要不同譯法時，在那個區塊的檔案裡寫一行就會蓋過文本層。

擋錯多一條：**文本層的原文對不上任何字串就報錯**——打錯一個字的話它會安靜地
一條都套不上，而症狀就只是「進度沒動」。

⚠ 文本層**不放**單一字母、數字與全大寫的關鍵字（`Y`／`N`／`CHAT`／`MAYOR`／
`FINSTER`…）。那些是玩家要輸入或選取的詞，翻掉就對不上了
（密語的比對程式碼還沒逆向，`docs/re/33` §4）。

## 5. 譯者規則（build 會擋）

| 規則 | 為什麼 | build 的行為 |
|---|---|---|
| **格數不得超過原文** | 訊息視窗只有 6 行 × 38 格（`docs/spec/10` §3） | 超過就是錯誤。⚠ **`place:` 與 `monster:` 例外**：那兩類的體例是「中文（英文）」，一定比原文長，改用「一行 38 格」當上限 |
| **控制碼要一字不差地保留** | `\x0A`／`\x0C`／`\x0E`／`\x0F` 是文字變形的分段（`docs/re/28`），少一個就會選錯段 | 數量對不上就是錯誤 |
| Big5 沒有的字要換掉 | 倚天字型是 Big5 排列 | 列出來並拒絕編譯 |
| 譯名以 `glossary.md` 為準 | 一致性 | 目前只警告 |
| **同一個 key 不得出現在兩個檔** | 跨檔重複會靜靜覆蓋掉一份翻譯，出來的是哪一份取決於檔名排序——**這種錯沒有任何症狀** | 列出兩個檔名並拒絕編譯 |
| **`untranslatable.tsv` 列的 key 不得有譯文** | 那些槽不是文字（未用槽的解碼雜訊、純控制碼，`docs/re/17` §1）。翻了就表示分類判斷錯了 | 列出來並拒絕編譯 |
| **`\x10` 後面的字元一字不改** | 那個字元同時是畫面上的第一個字**與點擊該列送出的按鍵**（`docs/re/43`）。翻成中文會讓滑鼠送出 Big5 的首位元組，而鍵盤比對的仍是原本的字母 | 逐一比對原文與譯文 `\x10` 後的字元序列，不同就拒絕編譯 |

**選項的寫法**：中文接在保留的字母後面，中間一個半形空白。

```
原文    \x11\x10Yes\x0D\x10No\x0D
中文    \x11\x10Y 是\x0D\x10N 否\x0D
```

### 專有名詞與地名的體例：中文（英文）

**店家招牌、地點名與敵人的專有名詞寫成「中文（英文）」**，英文放在全形括號裡
（使用者定案 2026-08-18）：

```
place:Ranger Ctr.       遊俠中心（Ranger Ctr.）
monster:Redhawk         紅鷹（Redhawk）
monster:Juv\x0Aenile\x0Aies\x0A   少年\x0A\x0A\x0A
```

理由是**對得上**：玩家拿著原版畫面、官方手冊或當年《軟體世界》的攻略時，
中文譯名對不回英文原名就查不到東西。普通名詞（`Juvenile`、`Rabid Dog`）
不加括號——那些不是要拿去對照的東西，加了只會佔掉訊息視窗的格子。

⚠ **哪一條算專有名詞是人的判斷**，機器分不出 `Woman` 與 `Weez`。
`TestProperNounFormat` 守的是「有括號的那些，括號裡就是原文」——
格式一致守得住，分類正確守不住。

**中文比英文密度高**，所以「格數不得超過原文」在實務上很鬆——
`You are being attacked!` 是 23 格、「你們遭到攻擊！」只有 8 格。
會擋到的通常是**漏刪原文的空白或誤加註解**。

## 6. `.cat` 的格式

```
magic "WLCAT\0"  (6 bytes)
版本   uint16
條數   uint32
逐條：keyLen uint16、key（UTF-8）、textLen uint16、text（UTF-8 bytes）
```

刻意做成最笨的格式：**Go 那邊只要會讀就好**，所有判斷都在編譯時做完了。

## 7. 執行期的行為

```
lang.Load(path)      載不到 → 整個中文關掉，遊戲跑英文（不得崩）
lang.Lookup(key)     沒有這條 → 回 false，呼叫者用原文
```

**沒翻的條目就顯示原文**，不留空白也不顯示 key——半成品的中文化要能玩。

## 8. 驗收條件

1. 抽取：`exe` 426 條 ＋ `blk` 4,401 條 ＝ **4,827 條**，與 `docs/re/17`／`18` 的數字一致。
2. Key 唯一，而且重跑抽取兩次結果完全相同（byte-for-byte）。
3. 控制碼 round-trip：TSV 的 `\xNN` 解回來要與原文的 bytes 完全相同。
4. 編譯：格數超過原文、控制碼數量不符、Big5 缺字、`\x10` 後字元被改、
   不可翻的槽被翻了，五種都要擋下來並指出是哪一條。
4b. **完成度有結論**：`4,827` 個槽扣掉 `translations/untranslatable.tsv`
   列的 21 個非文字槽 ＝ **4,806 條可翻條目**；編譯報告要能說出
   「可翻條目 N／4,806」，不留一個永遠減不掉的餘數。
5. Go 載入：查得到的回中文，查不到的回 false；**沒有 `.cat` 檔時遊戲照跑**。
6. 端到端：`-mode play` 踩到一個有翻譯的訊息格，畫面上出現中文。
