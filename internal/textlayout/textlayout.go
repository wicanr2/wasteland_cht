...
//
// 給**中文那條路**用：譯文是 UTF-8，一個字佔一個字元格而不是一個位元組，
...
//
// UTF-8 的前導位元組（0xC2–0xF4）與後續位元組（0x80–0xBF）**全部 ≥ 0x80**，
// 都落在控制碼 0x01–0x11 之外，所以逐位元組掃描是安全的。
// （這一點比 Big5 更安全：Big5 的尾位元組可以是 0x40–0x7E，與 ASCII 重疊。）
...
// Render 是 RenderBytes 的 UTF-8 字串版——中文那條路的入口。
//
// 控制碼的解讀是逐位元組的，而 UTF-8 的非 ASCII 位元組全部 ≥ 0x80，
// 不可能被誤認成控制碼，所以直接借用同一支引擎。
func Render(text string, opt Options) string {
	return string(RenderBytes([]byte(text), opt))
}
