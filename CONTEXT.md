...
**中文化完成**：目錄 **5,248／5,248** 條 ＋ 段落書 162 段，**而且畫面上真的是中文**——
戰鬥全程、設施選單與清單、角色管理、指令列訊息都走目錄（`docs/re/98` §2）。
那 5,248 條 ＝ 原版語料 4,827 條扣掉 21 個不可翻的槽（4,806）
＋ 重製版自己的 `ui:` 與店家招牌 `place:` 共 114 條
...
1. ~~翻譯本體~~ —— **已完成（5,248／5,248 可翻條目，`build_lang.py` 的輸出是權威數字）**。
   原版語料 4,827 個槽扣掉 21 個**不可翻**的槽（未用槽的解碼雜訊與純控制碼，
   `docs/re/17` §1）＝ 4,806 條，加上重製版自己的 `ui:` 與 `places:` 共 114 條
   ＋ 328 條明文敵人名字 ＝ 目錄裡的 5,248 條，全部翻完。
