...
1. **逃跑與隊形** —— 戰鬥剩下沒讀的兩塊（`docs/re/36` §4）。
2. **遭遇與戰鬥畫面** —— 規格 06 只做了「單次攻擊的結算」，回合結構、逃跑、
   隊形都還沒逆向。
3. **翻譯本體** —— 管線已通（`docs/spec/11`），剩下 4,807 條要翻。
   譯名以 `translations/glossary.md` 為準。
4. **段落手札** —— 段落編號是純文字（`docs/re/33`），中文化在文字層替換；
   但變體組與火星誘餌不能照抄成線性手札，要先決定呈現方式。
5. **設施的互動迴圈** —— 價格與規則已做（`docs/spec/09`），
