...
1. **翻譯本體** —— **執行檔九張表已翻完**（414 條；剩下 12 條是純控制碼與
   抽取工具的解碼雜訊，不該翻）。**剩 4,393 條全在 42 個地圖區塊**，
   以敘述文字為主，最大的幾塊是 `game1:3`（188）、`game1:2`（166）、
   `game1:6`（165）、`game2:23`（154）。譯名以 `translations/glossary.md` 為準。
