...
1. **翻譯本體** —— 目前 **4,213 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 614 條是各區塊自己的敘述文字**（已翻完：石英城 `game1:2/3/4/6`、
   ...
   沉睡者基地 `game2:15`、針岩城荒廢街 `game1:32`、達爾文基地 `game2:22`、
   維加斯街道 `game2:12`），
   最大的幾塊是 `game2:11`（55）、`game2:24`（51）、`game1:31`（51）。
