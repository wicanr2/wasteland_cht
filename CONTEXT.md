...
1. **翻譯本體** —— 目前 **4,420 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 407 條是各區塊自己的敘述文字**（已翻完：石英城 `game1:2/3/4/6`、
   ...
   下水道 `game2:24`、科奇斯基地內部 `game2:17`），
   最大的幾塊是 `game2:42`（45）、`game2:19`（43）、`game1:5`（41）。
