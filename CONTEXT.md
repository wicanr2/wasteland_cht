...
1. **戰鬥畫面** —— 規則層整條到位了（掃遭遇 → 生怪 → 下指令 → 行動順序 → 結算 →
   逃跑），缺的是呈現層：遭遇提示、戰鬥選單、訊息視窗。
2. **Hire／Weapon／Use／Load-unjam** —— 四支指令處理程式還沒逐行讀（`docs/re/38` §5）。
