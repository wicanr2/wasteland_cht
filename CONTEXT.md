...
| **音效** | **已解**：跑在 `int 08h` 上的位元組碼直譯器（126.36 Hz，每 6 tick 鏈回 BIOS）。**四個聲部是優先序仲裁不是混音**——PC 喇叭一次只發一個音，編號小的贏。四個 opcode（設欄位／計數器迴圈／換寫入基底／音符），每個聲部有滑音、線性分段封套、顫音、移調。**九首全在 `seg005` 的 864 bytes 裡**，不在外部檔案。⚠ 半音表的 **E 走音 −2.91 音分**（原始資料的錯字，`0x714F` 應為 `0x711E`），重製版照抄不修（`docs/re/44`、`docs/spec/08`、`internal/audio`）|
...
| [`docs/re/44-audio.md`](docs/re/44-audio.md) | 音效：計時器 ISR、四個聲部、位元組碼指令集、九首資料、走音的 E |
...
4. **音效 3／6／8 的觸發點** —— 資料與直譯器都解完了（`docs/re/44`），
   只剩這三首的呼叫端還沒定位。**不擋規格 08**：編號由呼叫端傳，音效層不猜。
