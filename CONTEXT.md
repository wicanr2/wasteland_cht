...
| **段落手札** | **已解＋已定案**：原版沒有段落機制，`Read paragraph N.` 就是一條普通敘述（`docs/re/33`）。引用表由 `tools/extract_paragraph_refs.py` 從**英文原文**抽成 `docs/re/generated/paragraph-refs.tsv`（83 條引用、82 個編號）——**不在執行期解析翻譯過的文字**，否則譯者的用字會決定遊戲讀不讀得到段落。手札**預設全開**（忠於紙本），分成正文（會被引用的 82 段）與附錄（其餘 80 段，含陷阱段 1／22／145），附錄標明是 1988 年防拷設計；**一段都不刪**（`docs/spec/19`）|
...
1. **翻譯本體** —— 目前 **3,288 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 1,539 條是各區塊自己的敘述文字**（已翻完：石英城 `game1:2/3/4/6`、
   高池鎮 `game1:10`、鐵路遊牧民族營地 `game1:8`、針岩城 `game1:33/34`、
   科奇斯基地的機器人工廠 `game2:20`、芬斯特意識 `game2:23`、
   蘑菇雲神殿 `game2:38`、維加斯 `game2:40`、黑桃賭場 `game2:41`），
   最大的幾塊是 `game2:13`（79）、`game2:21`（76）、`game1:27`（73）。
   譯名以 `translations/glossary.md` 為準；
   `python3 tools/untranslated.py` 列出剩下的區塊，加上 `<檔> <編號>` 倒出單一區塊。
2. **段落書的 162 段中文翻譯** —— 呈現方式已定案（`docs/spec/19`），
   但 `docs/paragraphs/` 目前只有英文原文，中文翻譯**還沒開始**。
   這是翻譯之外的另一批工作量。
