...
1. **翻譯本體** —— 目前 **3,720 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 1,107 條是各區塊自己的敘述文字**（已翻完：石英城 `game1:2/3/4/6`、
   ...
   維加斯 `game2:40`、黑桃賭場 `game2:41`、守護者堡壘 `game2:35/36`、
   野人村 `game1:49`），
   最大的幾塊是 `game1:9`（66）、`game1:1`（64）、`game2:25`（64）。
   譯名以 `translations/glossary.md` 為準；
   `python3 tools/untranslated.py` 列出剩下的區塊，加上 `<檔> <編號>` 倒出單一區塊。
2. **段落書的 162 段中文翻譯** —— **已翻 80 段（1–80），剩 82 段（81–162）**。
