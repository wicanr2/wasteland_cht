...
| **地圖事件處理** | **五支已讀完**：nibble 5 ＝ **寶箱**（第一次踩到才把類別擲成具體物品與數量，寫回記錄，之後不再變）、8 ＝ 多選一選單、9／12 ＝ 印訊息、10 ＝ 傳送。剩 1／2／4／6 只到入口（`docs/re/29`）|
...
| [`docs/re/00-function-index.md`](docs/re/00-function-index.md) | 函式索引（641 個，已分析 279）。讀任何 `sub_XXXXX` 前先查 |
