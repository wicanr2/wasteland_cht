...
（移動、時鐘、事件分派、44 個腳本指令、段落引用）都已解。
641 個函式裡人寫的筆記涵蓋 320 個。
完整缺口見 [`docs/re/00-remake-knowledge-gaps.md`](docs/re/00-remake-knowledge-gaps.md)。

**Remake 進度**：十三份規格 READY 並實作完成（資產、亂數、畫面與文字、移動與時鐘、
角色與存檔、戰鬥、世界事件、設施、中文排版、翻譯管線、回合結構、遭遇生成、戰鬥指令），
`cmd/wasteland -mode play` 已經可以從出廠存檔開始走地圖並存檔。
介面層 193 條中文已翻完並編進 `translations/zh-Hant.cat`。

### 7.1 下一輪要做的（照 §0 的節奏：讀 RE → 寫 spec → 實作）

1. **遭遇佇列 `ds:A96Fh` 是誰填的** —— 敵人怎麼生、血量怎麼定都已解
   （`docs/re/37`），但「哪一格會冒出遭遇」還沒讀，所以遭遇目前只能由呼叫端指定。
2. **戰鬥畫面** —— 規則層已到位（遭遇生成、指令、行動順序、結算、逃跑），缺呈現層。
3. **Hire／Weapon／Use／Load-unjam** —— 四支指令處理程式還沒逐行讀（`docs/re/38` §5）。
   「隊形」目前**沒有證據顯示存在**，不要先發明一個。
...
1. **`sub_15755` 的射程項** —— 隊伍傷害五項裡唯一沒定的那一項，
   程式碼裡以具名常數暫代成 0。（「距離」本身已解：`sub_19D4D` 查 `ds:CD0Dh`，
   `docs/re/37` §4；但 `sub_15755` 讀的是 `ds:46CCh`／`46CDh`，是另一條路。）
1b. **敵人資料表 `+0x04` 高 4 位、`+0x06`、`+0x07`** —— 8 個 byte 裡剩下的三處。
