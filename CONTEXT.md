...
| **滑鼠** | 規格 29。游標用原版 `CURS`（`internal/assets/curs.go`），**控制走 remake 自己的路**：點到哪一格就送那一格的字元、點地圖往那個方向走、右鍵取消。點擊一律翻成與鍵盤等價的輸入再走同一條 `Update` |
...
> T1–T31 都已完成（T13–T29 在 2026-08-17、T30–T31 在 2026-08-18），
...
| — | `Hire` 的結算**已解未接**（`docs/re/110`）| 流程完整讀完（section 17 取 NPC 記錄 → 抄 256 bytes → 魅力對決 → 槽表補位、7 人上限）。按 `H` 仍印「還沒做」|
| — | 移動計畫的三條分支怎麼選（`sub_19D2F`／`loc_12AA2`，`docs/re/101` §6）| remake 不模擬敵人在地圖上移動，三條分支一條都走不到；命中基礎值只看「有沒有計畫」 |
