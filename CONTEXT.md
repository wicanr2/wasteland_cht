...
| — | **遭遇生成器 `sub_16890` 的後半沒實作**（`docs/re/77`）：找空槽、擲種類與位置、把格子改成 nibble 15。缺的資料是遭遇表 `ds:AA60h`／`AA6Dh`／`AA7Ah` 的內容與 `sub_12A4C`／`sub_12A8D` 的索引算法 | **這一項擋玩得通**：105 步 0 次遭遇，隨機戰鬥整個不會發生 |
