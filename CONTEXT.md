...
| **敵人資料表** | **已解（8 個 byte 全部）**：`+0x00/+0x01` 基礎血量兼經驗值基值、`+0x02` 行動值、`+0x03` 傷害骰數、`+0x04` 低 4 位經驗值倍數 −1、`+0x05` 低 4 位**武器類別**（與物品表同一套編碼）高 4 位傷害基底、`+0x06` **敵人種類**（1 Animal／2 Mutant／3 Humanoid／4 Cyborg／5 Robot，397 筆全在 1–5，AI 對 1／4／5 有特判）、`+0x07` **未解**（兩個消費者兜不起來，見 `docs/re/37` §3.2）。⚠ `+0x04` **高 4 位沒有任何讀者**，資料裡有值但程式沒讀（`docs/re/37` §3.2）|
...
| 敵人資料 `+0x07` ＝ 代名詞索引 | 只追了一個消費者（`sub_12A4C` → `ds:A920h` → 文字碼 `0x0E`）就命名；同一個 byte 在 `0x1268F` 還被餵給 `sub_190A8`（資源切換），兩者兜不起來 | **未解**。兩條線索都記在 `docs/re/37` §3.2，在兜起來之前不給語意 |
...
1. **敵人資料 `+0x07` 到底是什麼** —— 兩個消費者兜不起來（`docs/re/37` §3.2）：
   `sub_12A4C` 拿它查 `ds:A920h`（79 筆、值只有 0／1／2）得到文字碼 `0x0E` 的
   him／her／it 選擇子；`0x1268F` 又把同一個 byte 餵給 `sub_190A8`（資源切換）。
   在兜起來之前不給語意。
2. `ds:46E1h` 觸發原地踏步的**頻率** —— 三個寫入點都讀完了（`docs/re/26` §1.2：
   進遊戲設 1、兩個長跳回主迴圈的出口設 0，**地圖迴圈裡沒有人清它**），
   但「主迴圈一輪等於玩家的什麼動作」要實機觀察，併進下面的 DOSBox 對拍。
3. 單鍵模式（記錄 `+0x00` bit7）在 42 張地圖裡實際用在哪些格子。
4. 追資源表 idx 7（無檔名，疑似存檔區）在硬碟模式下怎麼存取。
5. 在 DOSBox 跑解包版與原版對照，驗證解包正確性（`docs/re/02` §5），
   並補上規格 04 §8.5 的逐像素對拍。
6. 存檔尾段那 `0xA00` 未加密區的用途。
7. 物品資料 `+0x03` 的低 3 位（只有類別 8／9／13 的 bit0 是 1，沒讀到讀取端）、
