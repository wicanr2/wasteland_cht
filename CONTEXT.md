...
641 個函式裡人寫的筆記涵蓋 358 個。
完整缺口見 [`docs/re/00-remake-knowledge-gaps.md`](docs/re/00-remake-knowledge-gaps.md)。

**Remake 進度**：十八份規格 READY 並實作完成（資產、亂數、畫面與文字、移動與時鐘、
角色與存檔、戰鬥、世界事件、設施、中文排版、翻譯管線、回合結構、遭遇生成、戰鬥指令、
遭遇掃描、戰鬥畫面、指令處理程式、設施互動迴圈、段落手札），
`cmd/wasteland -mode play` 已經可以從出廠存檔開始走地圖並存檔。
介面層 193 條中文已翻完並編進 `translations/zh-Hant.cat`。

### 7.1 下一輪要做的（照 §0 的節奏：讀 RE → 寫 spec → 實作）

1. **翻譯本體** —— 目前 **3,914 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 913 條是各區塊自己的敘述文字**（已翻完：石英城 `game1:2/3/4/6`、
   高池鎮 `game1:10`、鐵路遊牧民族營地 `game1:8`、針岩城 `game1:27/33/34`、
   科奇斯基地 `game2:20`、達爾文村 `game2:21`、芬斯特意識 `game2:23`、
   沉睡者基地的複製人實驗室 `game2:13`、蘑菇雲神殿 `game2:38`、
   維加斯 `game2:40/25`、黑桃賭場 `game2:41`、守護者堡壘 `game2:35/36`、
   野人村 `game1:49`、農業中心 `game1:9`、石英城街道 `game1:1`），
   最大的幾塊是 `game2:39`（63）、`game2:15`（61）、`game1:32`（61）。
   譯名以 `translations/glossary.md` 為準；
   `python3 tools/untranslated.py` 列出剩下的區塊，加上 `<檔> <編號>` 倒出單一區塊。
2. **段落書的 162 段中文翻譯** —— **已翻 120 段（1–120），剩 42 段（121–162）**。
   管線已通：譯稿在 `translations/zh-Hant/paragraphs/*.md`（markdown，
   鏡射 `docs/paragraphs/` 的四個檔），`tools/build_paragraphs.py` 編成
   `translations/paragraphs-zh-Hant.cat`，`internal/play.Journal` 讀它。
...
3. **打字型密語的比對程式碼** —— `BIRD`／`DIPSTICK` 這類要玩家打字的答案，
   明文就在區塊字串表裡（`docs/re/33` §4），但拿它們去比對的程式碼還沒定位。
   **已經不擋中文化**：選項熱鍵的問題由 `docs/re/43` 解掉了。
