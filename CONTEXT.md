...
1. **翻譯本體** —— 目前 **3,017 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 1,790 條是各區塊自己的敘述文字**（已翻完：石英城 `game1:2/3/4/6`、
   高池鎮 `game1:10`、針岩城 `game1:33/34`、芬斯特意識 `game2:23`、
   蘑菇雲神殿 `game2:38`、維加斯 `game2:40`），最大的幾塊是
   `game2:20`（99）、`game2:41`（95）、`game1:26`（92）。
