...
| **按鍵巨集** | **已解**：F1–F10 播放、Alt+F1–Alt+F10 錄製，10 組 × 256 bytes（`ds:C062h`，狀態變數緊接在 `0xCA62` 之後）。錄的是**轉大寫之前**的原始鍵碼。這是玩家看得到的功能，規格要收（`docs/re/43` §6）|
...
2. 追資源表 idx 7（無檔名，疑似存檔區）在硬碟模式下怎麼存取。
3. 在 DOSBox 跑解包版與原版對照，驗證解包正確性（`docs/re/02` §5），
   並補上規格 04 §8.5 的逐像素對拍。
4. 存檔尾段那 `0xA00` 未加密區的用途。
5. 物品 70／71／72 為什麼沒有名字（`docs/re/45` §6）——三件都是類別 18、
