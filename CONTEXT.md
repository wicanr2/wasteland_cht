...
| **圖片動畫（A9）** | **控制流已解、資料解法還是假說**：參數區是兩張表——A 是每個動畫通道的**腳本**（格編號與延遲交錯、`0xFF` 循環），B 是每一格的**像素**；消費端是 overlay slot 17，依 BIOS 計時器 `0040:006C` 推進、**XOR 進 EGA 平面**。但照讀出來的解法**重建不出原版畫面**（126 個像素最好只降到 113），所以元素 word 的解法還沒對。下一步是讀 slot 0 建 `ds:8E09h` 的那一段（`docs/re/23` §5.2、§5.3）|
...
| A9 的疊法「已解」 | 只讀了 `sub_10B11` 的程式碼就下結論，**沒有回頭重建畫面驗證**。讀得懂每一條指令不等於重建得出輸出 | 照那個解法疊回去，126 個對不上的像素最好只降到 113。控制流仍然成立，元素 word 的解法降回假說（`docs/re/23` §5.3）|
...
| # | 還沒解的 | 為什麼不擋 |
|---|---|---|
| A9 | 圖片動畫的**元素 word 解法與疊法**（控制流已解，`docs/re/23` §5.2）| 靜態圖已經逐像素對過原版；動畫是額外的 |
...
| — | `ds:46EFh`（踩在輻射上的旗標）的**讀取端**：誰拿它扣血 | 寫入端已解（`docs/re/48` §5） |
