...
| **閒置不推進時間** | **實機確認**：站著不按鍵 45 秒，原版時鐘一分鐘都沒走。主迴圈在等按鍵時卡在 `sub_18E90` 的忙等迴圈裡，回不到頂端——**「站著讓時間流逝」不會發生**（`docs/re/47` §6.1）|
...
1. `IC0_9.WLF` 另外九張圖形各自是什麼（第 7 張 ＝ 隊伍圖示已解，`docs/re/47` §6.4）。
2. 存檔 round-trip 的實機驗收：改寫存檔之後原版還讀不讀得進去。
