...
| — | **商店與醫生怎麼進去**：32 筆設施記錄只有 2 筆有格子指到。**程式碼那一側已經封閉**——五個設施函式只透過 `ds:A4E0h` 跳表被呼叫，跳表只有 `sub_12C80` 讀，它只有兩個呼叫端（`docs/re/70` §4.2）。所以是資料側某一格在遊戲中才被改寫成設施；靜態改寫來源掃完只找到 2 處。資料側四條改寫來源也掃完（`docs/re/71` §5），五條資料側路徑全部掃完（`docs/re/71` §5）。**強假說**：`Ranger Ctr.` 的跳表索引是 3 ＝ 通用設施畫面 ＋ 兩項選單，商店與醫生是從那個選單進去的，不是由格子分派（`docs/re/71` §5.5）。下一步是找 `ds:CE42h`／`CE44h` 的覆寫點 | **這一項擋玩得通**：商店、醫生、大部分訓練師目前到不了 |
