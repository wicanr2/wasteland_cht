...
| — | **商店與醫生怎麼進去**：32 筆設施記錄只有 2 筆有格子指到。**程式碼那一側已經封閉**——五個設施函式只透過 `ds:A4E0h` 跳表被呼叫，跳表只有 `sub_12C80` 讀，它只有兩個呼叫端（`docs/re/70` §4.2）。所以是資料側某一格在遊戲中才被改寫成設施；靜態改寫來源掃完只找到 2 處。資料側四條改寫來源也掃完（`docs/re/71` §5），五條資料側路徑全部掃完（`docs/re/71` §5）。實機解出 `Ranger Ctr.` 的完整入口：**旁邊的 nibble 12 先把設施格改寫成傳送格**（`docs/re/72` §2）。所以「幾筆設施有格子指到」這個統計沒有意義——格子兩邊都在動。下一步：nibble 12 批次表裡 `flags & 1`（相對隊伍座標）那一族單獨逐筆看 | **這一項擋玩得通**：商店、醫生、大部分訓練師目前到不了 |
