...
1. **翻譯本體** —— 目前 **2,360 / 4,827**。執行檔九張表已翻完（414 條；剩 12 條是
   純控制碼與解碼雜訊，不該翻）；地圖區塊的共用文本（戰鬥動詞、開門訊息）
   走**文本層** `_shared.tsv`，185 條翻譯涵蓋 1,425 個 key。
   **剩 2,447 條是各區塊自己的敘述文字**（石英城的 `game1:2/3/4/6` 已翻完），
   最大的幾塊是 `game2:23`（121）、`game2:38`（118）、`game1:10`（114）、
   `game2:40`（107）、`game1:33`（106）。
