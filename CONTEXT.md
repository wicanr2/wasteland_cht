...
| **戰鬥指令 `H` 雇用** | `docs/re/110`、規格 17 §4。遭遇記錄 `+0x09` 的 bit1 ＝ 可雇用、高 4 位 ＝ section 17 的 NPC 記錄編號 → 整筆 256 bytes 抄進隊伍 → 魅力對決 → 7 人上限 |
| **攻略斷言對照** | `docs/walkthrough/swm-005/mechanics-claims.md`：軟體世界攻略的 31 條機制斷言逐條拿逆向驗過（相符 21、有出入 4、未定 6）|
...
**中文化完成**：目錄 **4,915／4,915** 條 ＋ 段落書 162 段，**而且畫面上真的是中文**——
戰鬥全程、設施選單與清單、角色管理、指令列訊息都走目錄（`docs/re/98` §2）。
那 4,915 條 ＝ 原版語料 4,827 條扣掉 21 個不可翻的槽（4,806）
＋ 重製版自己的 `ui:`／`places:` 共 109 條。**`tools/build_lang.py` 的輸出是權威數字**——
...
> T1–T35 都已完成（T13–T29 在 2026-08-17、T30–T35 在 2026-08-18），
> **現在沒有一項擋著把遊戲玩完**。換 session 先讀那一份。

下面兩條是已經結案的紀錄，留著是因為它們是驗收數字的來源。


1. ~~翻譯本體~~ —— **已完成（4,915／4,915 可翻條目，`build_lang.py` 的輸出是權威數字）**。
...
| — | 戰鬥指令階段的**肖像畫的是誰**（`docs/re/105` §2）| 沒有敵人時實機畫的是遊俠（已對拍）；有敵人時是不是敵人肖像還缺一張遭遇截圖。肖像框是版面的最後一塊，remake 還沒畫 |
| — | **卡彈的武器名要反白**（`docs/re/111`）| `sub_19E2A` 解出來了＝反白開（`ds:4678h`）。bit7 ＝ 卡彈已接，**反白沒接**——高解那條路還沒有反白繪製 |
| — | 出貨資料裡**沒有一筆遭遇記錄把可雇用那個位元設起來**（`docs/re/110` §5.2）| `Hire` 的結算已解並接上（`internal/play/hire.go`），流程走得通；缺的是「遊戲裡什麼時候才會出現可雇用的遭遇」——遭遇記錄 `+0x09` 還有六個位元沒解 |
| — | 戰鬥中敵人在地圖上**移動本身**還沒實作（`docs/re/87` §2）| 三條分支的判定條件已經解完（`docs/re/101` §6：有射程武器 → 換位置、種類 1 → 看距離、其餘看士氣）。接上移動之後命中基礎值 60 那一支才會亮起來 |
