...
641 個函式裡人寫的筆記涵蓋 391 個（250 個沒碰過，多半是無呼叫端的死碼與 C 執行期）。
...
| — | 戰鬥的兩處暫代（`docs/re/81` §3）：命中沒接射程與距離懲罰、戰鬥中敵人打誰的規則未解（`sub_15036` 是**地圖上的移動**不是目標選擇，`docs/re/87`），remake 用「還能行動的人裡第一個」；訊息主詞已補（`docs/re/86`） | 打得完、結算正確；缺的是精確度與訊息 |
