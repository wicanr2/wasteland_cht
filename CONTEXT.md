...
| **翻譯目錄** | **管線已通**：`tools/extract_strings.py` 抽出 **4,827 條原文**（426 ＋ 4,401，與 `docs/re/17`／`18` 吻合）→ 人翻的 UTF-8 TSV → `tools/build_lang.py` 編成 Big5 `.cat`。**編譯時擋三種錯**：格數超過原文、控制碼數量不符、Big5 缺字。Go 只讀 `.cat`，不依賴任何編碼函式庫。端到端驗過：踩到有翻譯的格子畫面上出現中文。目前 20 條種子翻譯，其餘 4,807 條是人的工作 |
...
3. **遭遇與戰鬥畫面** —— 規格 06 只做了「單次攻擊的結算」，回合結構、逃跑、
   隊形都還沒逆向。
4. **翻譯本體** —— 管線已通（`docs/spec/11`），剩下 4,807 條要翻。
