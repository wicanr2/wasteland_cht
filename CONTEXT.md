...
| **戰鬥回合** | **骨架完成**：戰場是三組 × 每組 10 個（`ds:46C8h` 的 HP 陣列之後 `+0x14` 起是「這回合還沒行動」旗標），行動值 ＝ 2d6 續擲 ＋ 某欄位 × 8，寫進 `ds:7931h` 的**固定格子**行動表。雙方排在同一張表。排序規則與逃跑未解，實作以「行動值大的先動」暫代並註明（`docs/re/36`、`docs/spec/12`）|
...
1. **戰鬥的排序規則與逃跑** —— 行動值怎麼算已解，排序那一段還沒讀（`docs/re/36` §3）。
