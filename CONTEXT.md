...
| **規格（G2）** | **七份 READY**：`docs/spec/01`（資產與資料格式）、`02`（亂數與擲骰）、`03`（畫面與文字）、`04`（移動與時鐘）、`05`（角色與存檔）、`06`（戰鬥）、`07`（世界事件）。只剩 `08`（音效）等 F2 的位元組碼指令集 |
...
| **翻譯目錄** | **管線已通**：`tools/extract_strings.py` 抽出 **4,827 條原文**（426 ＋ 4,401，與 `docs/re/17`／`18` 吻合）→ 人翻的 UTF-8 TSV → `tools/build_lang.py` 編成 Big5 `.cat`。**編譯時擋三種錯**：格數超過原文、控制碼數量不符、Big5 缺字。Go 只讀 `.cat`，不依賴任何編碼函式庫。端到端驗過：踩到有翻譯的格子畫面上出現中文。**已翻 193 條**（35 個技能、95 件物品、50 個階級、8 種疾病——全部是介面層、有譯名表可依），其餘 4,634 條以敘述文字為主 |
...
| **戰鬥回合** | **骨架完成**：戰場是三組 × 每組 10 個（`ds:46C8h` 的 HP 陣列之後 `+0x14` 起是「這回合還沒行動」旗標），行動值 ＝ 2d6 續擲 ＋ 某欄位 × 8，寫進 `ds:7931h` 的**固定格子**行動表。雙方排在同一張表。**誰先動也解了**：原版沒有排序演算法，是「線性掃一遍挑最大、做完清零、再掃」，而且比較是 `A ≥ B` 所以**平手取後面的格子**——隊伍排在敵人後面，平手時隊伍先動。剩逃跑與隊形未解（`docs/re/36`、`docs/spec/12`）|
| **戰鬥** | **已實作**：命中判定（兩邊方向相反、夾在 100）、敵方傷害（基底 ＋ Nd6）、隊伍傷害（五項相加、沒有骰）、護甲吸收 ＝ N 顆 d6、角色 CON 可為負與五級傷勢、敵人 HP 夾在 0、擊殺經驗值。距離項未解，程式碼裡明寫是暫代 |
| 規則層其餘 | 回合結構、逃跑、隊形都還沒逆向；音效（規格 08）等 F2 的位元組碼指令集 |
...
**RE 完成度**：資料格式、文字與資產層打通；規則層六塊（戰鬥／屬性／效果／商店／
升級／檢定與技能成長）與世界互動層（移動、時鐘、事件分派、44 個腳本指令、
段落引用）都已解。641 個函式裡人寫的筆記涵蓋 293 個。
完整缺口見 [`docs/re/00-remake-knowledge-gaps.md`](docs/re/00-remake-knowledge-gaps.md)。

**Remake 進度**：七份規格 READY 並實作完成（資產、亂數、畫面與文字、移動與時鐘、
角色與存檔、戰鬥、世界事件），`cmd/wasteland -mode play` 已經可以從出廠存檔開始
走地圖並存檔。

### 7.1 下一輪要做的（照 §0 的節奏：讀 RE → 寫 spec → 實作）

1. **敵人 HP 的來源** —— `ds:46C8h` 前幾個 byte 有兩種讀法還沒釐清
   （`docs/re/36` §5）。**這一條沒解，遭遇就接不上戰鬥**——不編一個 HP 出來。
2. **逃跑與隊形** —— 戰鬥剩下沒讀的兩塊。
3. **遭遇與戰鬥畫面** —— 規格 06 只做了「單次攻擊的結算」，回合結構、逃跑、
   隊形都還沒逆向。
4. **翻譯本體** —— 介面層 193 條已翻完；剩 4,634 條以敘述文字為主。
   譯名以 `translations/glossary.md` 為準。
5. **段落手札** —— 段落編號是純文字（`docs/re/33`），中文化在文字層替換；
   但變體組與火星誘餌不能照抄成線性手札，要先決定呈現方式。
6. **設施的互動迴圈** —— 價格與規則已做（`docs/spec/09`），
   買賣挑物品、逐點付錢治療這些流程還沒逐行讀。

### 7.2 還沒解的逆向（不擋目前的實作）

1. **`sub_15755` 的距離／射程項** —— 隊伍傷害五項裡唯一沒定的那一項，
   程式碼裡以具名常數暫代成 0。
2. **`ds:916Bh`** —— 檢定要不要給經驗值的開關，目前暫代成一律給。
3. **密語輸入的比對程式碼** —— 決定中文化要不要改輸入層（`docs/re/33` §4）。
4. **音效（F2）** —— `sub_1CD52` 那套位元組碼的指令集與曲目資料在哪；
   規格 08 卡在這裡。
5. 逐一解 overlay 其餘的未解 slot（26 個裡已解 12 個）。
6. 追資源表 idx 7（無檔名，疑似存檔區）在硬碟模式下怎麼存取。
7. 在 DOSBox 跑解包版與原版對照，驗證解包正確性（`docs/re/02` §5），
   並補上規格 04 §8.5 的逐像素對拍。
8. 存檔尾段那 `0xA00` 未加密區的用途。
