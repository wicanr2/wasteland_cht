...
| **圖片動畫（A9）** | **已解**：`ALLPICS` 交錯的參數區是兩張表——A 是每個動畫通道的**腳本**（格編號與延遲交錯、`0xFF` 循環），B 是每一格的**像素**。overlay slot 17 依 BIOS 計時器`0040:006C` 逐格 **XOR 進 EGA 平面**（不是疊回圖片緩衝區，這一點是所有實驗成敗的關鍵）。元素 word ＝ 相位 ＋ 長度 ＋ `ax ÷ 12` 的列欄（一列 12 bytes）（`docs/re/23` §5.2）|
...
| # | 還沒解的 | 為什麼不擋 |
|---|---|---|
| — | **圖片動畫**（原 A9）**已解**但重製版還沒畫（呈現層下一輪，`docs/re/23` §5.2）| 靜態圖已經逐像素對過原版 |
