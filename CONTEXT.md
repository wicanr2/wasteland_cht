...
## 0.1 新版美術三模式（使用者定案 2026-08-21）

新增 `original`、`faithful-hd`、`reimagined` 三種可在遊戲內原子切換的呈現模式。
`faithful-hd` 維持 4:3 與原版格線；`reimagined` 採固定 3/4 俯視、16:9 響應式 HUD、
有上限的擴張視野與最多七人的個別角色動畫。兩種新版模式共用寫實、髒舊、低彩度
的現代像素廢土方向；tileset、sprite、82 張分層場景、標題與結局素材全部由 AI
產生及整理。遊戲規則、事件、碰撞與存檔維持單一來源。完整契約與發佈邊界見
[`docs/spec/30-modern-art-themes.md`](docs/spec/30-modern-art-themes.md)。

## 0.2 復古／現代雙配樂（使用者定案 2026-08-21）

1988 DOS 原版沒有背景音樂，因此玩家口語所稱「原版／新版」在介面中採誠實標示：
既有專案自作、MT-32 算音的十首稱「復古版」，同一批自作曲重新配器與現代混音的
版本稱「現代版」。F2 可即時切換，原版 PC 喇叭音效仍是獨立開關；設定不寫進
原版存檔。檔案契約、退回行為與權利邊界見 [`docs/spec/31-dual-score.md`](docs/spec/31-dual-score.md)。
...
- 玩家已定案三模式：`original`、固定 4:3 的 `faithful-hd`、以及唯一會隨視窗
  重繪的 16:9 `reimagined`；F2 設定按 `V` 原子循環，失敗維持舊模式。
- `faithful-hd` 交付 1,157 個資產；`reimagined` 交付 1,905 個資產，後者包含
  1,051 張 3/4 地形、82 場景、標題／結局、七名角色的八方向六種狀態，以及
  四類武器獨立圖層。兩包皆以 manifest 尺寸與 SHA-256 驗證。
- 三模式共用同一 `game.World` 與存檔；新版呈現不作為原版 parity 證據。
...
| **翻譯目錄** | **管線已通**：`tools/extract_strings.py` 抽出 **4,827 條原文**（426 ＋ 4,401，與 `docs/re/17`／`18` 吻合）→ 人翻的 UTF-8 TSV → `tools/build_lang.py` 編成 **UTF-8 `.cat`**（v2）。**編譯時擋五種錯**：格數超過原文、控制碼數量不符、Big5 缺字（倚天畫不出來的字，這關沒放鬆）、`\x10` 後的按鍵被翻掉、不可翻的槽被翻了。Go 只讀 `.cat`，**整條管線 UTF-8，只在畫字那一刻轉 Big5**（`render.DrawRune`，唯一一處；`docs/utf8-experiment.md`）。端到端驗過：踩到有翻譯的格子畫面上出現中文。**已全部翻完：5,250／5,250 可翻條目**（原版語料 4,827 槽扣掉 21 個不可翻的槽——未用槽的解碼雜訊與純控制碼，清單在 `translations/untranslatable.tsv`，build 會擋「不該有譯文的 key 被翻了」）。**另外三類不在字串表裡**：`ui:` 介面文字、`place:` 店家招牌、`monster:` 敵人名字（328 條，`tools/extract_monster_names.py` 抽出來，`docs/re/114` §6）。共用文本層 `_shared.tsv` 的 185 條涵蓋 1,425 個 key |
| **中文排版** | **已決策並實作**：內部畫布 **960 × 600**（原解析乾淨 3×），原版素材 nearest 放大、倚天直繪不縮字（`rulebook/81`）。一個中文字剛好佔原版一個字元格，所以 `docs/re/25` 的座標完全不用重算、訊息視窗仍是 6 行 × 38 格。字型優先 24 點（`STDFONT.24` ＋ `SPCFONT.24` ＋ 半形 `ASCFONT.24`），只有 15 點時照原尺寸畫在格子中央；Big5 分區索引已過 oracle（「一」是一條橫線、「中」「猴」可辨識、全形標點不落 fallback）。**字型檔玩家自備，沒有時遊戲照跑英文**（`docs/spec/10`）。**行距 0（使用者定案 2026-08-20）**：24 點漢字的墨跡填滿整格，而字元格就是 24 px，所以要有行距只能換 15 點字或加高畫布——兩條都會動到別的東西，定案是維持整三倍的關係|
...
641 個函式裡人寫的筆記涵蓋 512 個（129 個沒碰過，多半是無呼叫端的死碼與 C 執行期）。
...
**中文化完成**：目錄 **5,250／5,250** 條 ＋ 段落書 162 段，**而且畫面上真的是中文**——
戰鬥全程、設施選單與清單、角色管理、指令列訊息都走目錄（`docs/re/98` §2）。
那 5,250 條 ＝ 原版語料 4,827 條扣掉 21 個不可翻的槽（4,806）
＋ 重製版自己的 `ui:` 與店家招牌 `place:` 共 116 條
...
**人工試玩第一輪的回報已處理**（2026-08-25，明細在 `WORKLIST.md`「2026-08-25
...
1. ~~翻譯本體~~ —— **已完成（5,250／5,250 可翻條目，`build_lang.py` 的輸出是權威數字）**。
   原版語料 4,827 個槽扣掉 21 個**不可翻**的槽（未用槽的解碼雜訊與純控制碼，
   `docs/re/17` §1）＝ 4,806 條，加上重製版自己的 `ui:` 與 `places:` 共 116 條
   ＋ 328 條明文敵人名字 ＝ 目錄裡的 5,250 條，全部翻完。
