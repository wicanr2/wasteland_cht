...
| **文字編碼** | **已解**：執行檔的遊戲文字是 **5-bit 打包 ＋ 60 字元對照表**，九張表合計 **442 個字串**（技能、物品、階級、疾病、介面、**結局敘述**）。`tools/decode_text.py`（`docs/re/17`）|
...
| [`docs/re/00-function-index.md`](docs/re/00-function-index.md) | 函式索引（641 個，已分析 150）。讀任何 `sub_XXXXX` 前先查 |
...
| [`docs/re/17-packed-text.md`](docs/re/17-packed-text.md) | 5-bit 打包文字的格式與解碼器、九張字串表、單複數與性別機制、名片欄位對照 |
...
2. **存檔內部欄位（C2）** —— 角色記錄定址與大半欄位已解（`docs/re/15`），
   ...
   以及位移用算的那 127 處存取。
3. **MSQ 區塊標頭 `+0x00` 的打包文字** —— 地圖內文字的所在，中文化的主體。
   執行檔那套解法套不上去（`docs/re/17` §6）。
