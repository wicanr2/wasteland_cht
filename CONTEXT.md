...
| **敵人資料表** | **已解（8 個 byte 全部）**：`+0x00/+0x01` 基礎血量兼經驗值基值、`+0x02` 行動值、`+0x03` 傷害骰數、`+0x04` 低 4 位經驗值倍數 −1、`+0x05` 低 4 位**武器類別**（與物品表同一套編碼）高 4 位傷害基底、`+0x06` **敵人種類**（1 Animal／2 Mutant／3 Humanoid／4 Cyborg／5 Robot，397 筆全在 1–5，AI 對 1／4／5 有特判）、`+0x07` **代名詞索引**（查 `ds:A920h` → 文字碼 `0x0E` 的 him／her／it）。⚠ `+0x04` **高 4 位沒有任何讀者**，資料裡有值但程式沒讀（`docs/re/37` §3.2）|
...
1. **`ds:A920h` 是「什麼的索引」** —— 敵人資料 `+0x07` 拿它查代名詞（`docs/re/37` §3.2），
   索引值域 4–78，看起來像全域的敵人種別編號，但那張編號表還沒找到。
2. **自動步（方向碼 4）的完整行為** —— `ds:916Bh` 的語意已解（`docs/re/32` §7.1，
   ＝「這一步是不是玩家自己走的」，防刷），但 `sub_1651A(4)` → `sub_1649E`
   那條路怎麼走完還沒讀；remake 目前只有玩家步。
3. **打字型密語的比對程式碼** —— `BIRD`／`DIPSTICK` 這類要玩家打字的答案，
   明文就在區塊字串表裡（`docs/re/33` §4），但拿它們去比對的程式碼還沒定位。
   已排除兩條線索：**全檔沒有任何 `cmpsb`／`scasb`**（比對是手寫迴圈或不存在），
   而 `sub_1C9DE` 曾被當成「名字輸入樣板」，實際上是**發起始裝備**（`docs/re/21` §5.1）。
   連帶未解：**角色名字是怎麼輸入的**——`sub_1C6C9` 裡沒有讀字母進緩衝區的迴圈。
   **已經不擋中文化**：選項熱鍵的問題由 `docs/re/43` 解掉了。
4. 逐一解 overlay 其餘的未解 slot（26 個裡已解 12 個）。
5. 追資源表 idx 7（無檔名，疑似存檔區）在硬碟模式下怎麼存取。
6. 在 DOSBox 跑解包版與原版對照，驗證解包正確性（`docs/re/02` §5），
   並補上規格 04 §8.5 的逐像素對拍。
7. 存檔尾段那 `0xA00` 未加密區的用途。
8. `sub_1CB67` 遞減的 `word_29F70` 是什麼計數器（`docs/re/44` §8）。
9. 物品資料 `+0x03` 的低 3 位（只有類別 8／9／13 的 bit0 是 1，沒讀到讀取端）、
   投擲爆裂物那條路（`0x12E40`）的 `x` 是什麼、物品 70／71／72 為什麼沒有名字
   （`docs/re/45` §6）。
