...
| **G4 — 接線** | 那份筆記在 [`docs/re/00-wiring-status.md`](docs/re/00-wiring-status.md) 登記，狀態與程式碼一致 | 接線表一列 + `TestWiringStatus` 綠 |

**G4 不是形式。** 前三道各自都會綠，而結論仍然可以躺在筆記裡沒人用：
命中公式的 Agility 與對手行動值早就解在 `docs/re/21`、`37`，敵人目標選擇
早就解在 `docs/re/20` §1.1，而 remake 三處都傳著寫死的常數——
**編得過、測得過、玩得動**（`docs/re/88`、`89`）。所以：

- **每寫完一份 `docs/re/NN`，同一輪就要在接線表登記**（已接／未接／不適用）。
  `TestWiringStatus` 雙向都會紅：沒登記會紅、說接了卻沒人引用會紅、
  說沒接卻被引用了也會紅。
- **每解出一個欄位或常數，回頭 grep 誰在等它。** 尤其看呼叫端：
  **參數列裡「所有呼叫端都傳同一個字面常數」的位置，就是還沒接上的訊號。**
- 引用寫在**用到那個結論的地方**，不是集中在檔頭——讀程式碼的人當場看得到依據，
  機器也數得到。
...
- **動手之前先查 `docs/re/00-master-index.md`（RE 總表）。** 位址換算、資料格式、
  ...
  四份 `00-*` 的分工：**總表**＝已知的事實；`00-remake-knowledge-gaps.md`＝還缺什麼；
  `00-function-index.md`＝641 個函式誰解過；
  **`00-wiring-status.md`＝已經解出來的，接上了沒有**（G4，見 §0）。
- **要動某個機制之前，先在接線表查它那一列。** 標「已接」就順著引用點去讀現有實作，
  不要重寫一遍；標「未接」就照理由欄接上去。
...
- 中文一律點陣字（倚天），字型檔玩家自備、不隨專案散布。**優先 24 點**
  （`STDFONT.24` ＋ `SPCFONT.24` ＋ 半形 `ASCFONT.24`），畫布 960×600；
  只有 15 點時照原尺寸畫在格子中央。**英數用倚天自己的半形字，
  不要拿遊戲的 8×8 字模放大**——放大後筆劃比中文粗，混排會露餡。
...
- **文件要寫現況，不寫「當初怎麼錯的」。** 斷言被推翻就把正文改寫成正確答案，
  ...
  **那張表只放「已經知道錯、還不知道對」的**：正確答案一旦定案就把那一列刪掉，
  教訓留在該份筆記的「這一輪學到的（寫成規則）」。留著會變成第二個要維護的真相。
  教訓寫成規則，不要寫成事件敘述。
...
docs/re/00-wiring-status.md         接線狀態（RE 結論 → remake 有沒有用上）
docs/formats/     資料格式規格
...
一句話現況：四道閘門都走過了——資料格式、文字層、規則層與世界互動層已解，
