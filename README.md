...
RE 筆記 122 份，每一份都在[接線表](docs/re/00-wiring-status.md)登記
「這個結論 remake 用上了沒有」——**編得過、測得過、玩得動的結論躺在筆記裡沒人接**
是這個專案最常犯的錯，所以用一支測試雙向守著。**現在表上一份「未接」都沒有**：
最後三份（條件閘扣血的護甲吸收、接戰值、畫面外框與輻射計量表）在 2026-08-20 接上。

**沒有一項擋著把遊戲玩完。** 畫面上該有的框、標籤與計量表都齊了；
剩下的是框邊標籤的滑鼠熱區、一個要定案的版面決策（中文行距），
以及還沒讀完的逆向（存檔輪替的序號、蓋氏計數器的滴答聲）。
...
相符 25、有出入 5、未定 1）。剩那一條缺的只是「攻略說的『廢坑』是哪一張地圖」。
