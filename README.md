...
| ![結局](docs/images/05-ending.png) | ![後日談](docs/images/06-epilogue.png) |
| **結局**：`END.CPA` 兩段全解——畫面 ＋ 一份 15 格的動畫腳本 | **後日談**：《遊俠史》第二卷的獻詞，**原版玩不到**，收進手札保存 |
...
右上角視窗用亂數擲出一組屬性，不滿意就按空白鍵重擲，滿意了按 RETURN 取名字（不超過 13 個字）。
...
用無線電呼叫總部領表揚與升級、存檔。**走進 Base Cochise 最深處會跑出結局**。
訊息是中文的。
...
| 開場與結局 | 標題畫面 → `Start` → 地圖；結局的畫面、15 格動畫、四段敘述與死者點名 |
...
RE 筆記 96 份，每一份都在[接線表](docs/re/00-wiring-status.md)登記
「這個結論 remake 用上了沒有」——**編得過、測得過、玩得動的結論躺在筆記裡沒人接**
是這個專案最常犯的錯，所以用一支測試雙向守著。

剩下的工作分成三類，**沒有一項擋著把遊戲玩完**：原版就沒有消費端的
（`CURS`、`TRANSTBL`）、走不到的程式碼路徑（17 個沒有格子指到的腳本 opcode、
開場的 attract mode）、文件與驗收（說明書、手冊、攻略、實機對拍、抽樣試玩）。
