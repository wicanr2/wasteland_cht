...
| 角色（建立重擲、升級、技能、裝備、裝填、卸卡彈、交給隊友） | 照原版 | 含 `+0x25`／`+0x2E` 兩個晚期才解出的欄位 |
...
| 物品重排 | 未做 | 原版清單框架的 `0xFE` 那條（`docs/re/131` §3）；不影響任何判定 |
| 按鍵巨集（F1–F10 錄放） | 未做 | 功能鍵讓給重製版的說明／設定／快速存讀檔；巨集資料照樣 round-trip |
| 複製保護 | 不移植 | 設計決策：段落書直接做進手札 |
| `USE` 的 Medic／Doctor 特例 | 未做 | `loc_13D82` 那條路未解（`docs/re/92` §5）；醫療走設施與戰鬥既有路徑 |
