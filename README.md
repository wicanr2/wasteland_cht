...
目前可在遊戲內切換三種畫面模式：保留原貌的「原版」、以新版圖磚與角色重繪但
維持 4:3 的「忠實高畫質」，以及會隨視窗尺寸動態重繪的「全面重構」。
原版模式的中文走倚天 24×24 點陣字、英數走倚天同高的半形 `ASCFONT.24`，
基準畫面是 960×600（原版 320×200 的三倍）；原版資料仍由玩家自備。
...
| 全面重構（動態寬螢幕） | 忠實高畫質（4:3） |
|---|---|
| ![全面重構的荒漠地圖](docs/images/remake-reimagined-map.png) | ![忠實高畫質的荒漠地圖](docs/images/remake-faithful-hd-map.png) |
| 新版場景、圖磚與角色會配合視窗尺寸重新配置與繪製 | 保留原作 4:3 構圖，替換為新版圖磚與角色 |

![全面重構模式的戰鬥畫面](docs/images/remake-reimagined-combat.png)

全面重構模式同樣會替換戰鬥與不同場景的畫面；另外兩種模式固定維持 4:3，避免
縮放改變原作介面構圖。

### 復古／現代雙配樂

1988 年 DOS 原版沒有背景音樂，只有九段 PC 喇叭音效；本專案因此不把重製配樂
誤稱為「原版配樂」。完整版內含兩套專案自作配樂：復古版以 MT-32 音色呈現，
現代版以同一組主題動機重新編曲。遊戲中按 `F2` 開啟設定，再按 `B` 即時切換
「復古配樂／現代配樂」；`M` 控制開關，方向鍵調整音量。缺少現代版個別曲目時，
引擎會安全退回同場景的復古版，不修改原版存檔格式。
...
| **地圖**：底部是原版的指令列，熱鍵字母照原版不跟著翻譯走 | **訊息**：5,250 條可翻字串已全部翻完 |
...
| ![名單](docs/images/09-roster.png) | ![角色畫面](docs/images/10-charview.png) |
| **名單**：空白鍵在地圖上展開隊伍名單（原版 `ds:46B9h`），一有新訊息自動收回；點名片行開那個人的角色畫面 | **角色畫面**：按 `1`–`7` 檢視隊員，裝備、裝填、卸卡彈、把物品交給隊友——交給還有原版那個「不肯交易」的檢定 |
...
## 1988 年 12 月，台灣就有人在寫這款遊戲的攻略

比軟體世界那本說明書早一年半、與美國原版同年，《精訊電腦》第 27 期登了一篇
Apple II 版的攻略。標題是一副對聯：

> 傲嘯荒漠　遊俠仗義江湖行
> 浮萍半生　神鎗漂泊走天涯
>
> —— 精訊電腦〈遊騎兵手記〉，1988 年 12 月（[逐頁轉錄](docs/walkthrough/jingxun-027/)）

體例與 1990 年那本完全不同：它是**第一人稱的手記**，以隊上繪圖員 Fox 的口吻寫成，
四個人物有名字、有個性，遇敵、談判、賭博、隊友差點被怪物當午餐都寫進去，
攻略資訊藏在敘事裡。作者「銀狐」自己畫了六張地圖——世界地圖、高池鎮、農業中心、
沙漠遊牧民族、礦坑、石英城與酒吧。

它把 Ranger 譯成**遊騎兵**（軟體世界譯「遊俠」，代理版封面寫「荒野遊俠」），
三種叫法都留著。

當年的敘述與今天倒出來的資料還對得上。它寫礦坑裡那口箱子
「只帶走一條繩索，並且每人拿了一個防毒面具」，而資料裡那一筆寶箱記錄是：

```
| 43 | game1 | 2 |  | 54 Rope、47 Gas mask ×4 |
```

一條繩索、四具面具，四人隊伍剛好一人一個——**Apple II 版的攻略與 DOS 版的資料
互相印證，連件數都一樣**。14 條機制斷言逐條驗過的結果在
[`mechanics-claims.md`](docs/walkthrough/jingxun-027/mechanics-claims.md)
（相符 9、版本差異 2、有出入 1、未定 2）；那篇〈江湖經驗〉記的換資料片重來與
`Ctrl`－`R` 重排是**磁片版的玩法**，DOS 版沒有，所以對照表多一種狀態叫「版本差異」。
...
用無線電呼叫總部領表揚與升級、把路上遇到的 NPC 雇進隊伍（原版資料裡有 14 個雇得到的人）、
在地圖上展開隊伍名單與角色畫面（裝備、裝填、卸卡彈、把東西交給隊友）、
踩到布袋撿東西（第一次踩到才擲內容，與原版同一套 lazy 生成）、存檔。**遊戲玩得完**：科奇斯基地反應爐層的
...
| 中文化 | 可翻字串 5,250／5,250（原版語料 4,806 ＋ 重製版介面 116 ＋ 明文名字 328：敵人名字與店家招牌）、段落書 162／162 |

敵人的名字用的是**原版那張明文名字表**——畫面上寫「少年」「女子」「鈦爪獸」，
不是一律「人形生物」。**專有名詞與地名寫成「中文（英文）」**
（`遊俠中心（Ranger Ctr.）`、`紅鷹（Redhawk）`），這樣玩家拿著原版畫面、
官方手冊或當年《軟體世界》的攻略時對得回去。
...
RE 筆記 132 份，每一份都在[接線表](docs/re/00-wiring-status.md)登記
「這個結論 remake 用上了沒有」——**編得過、測得過、玩得動的結論躺在筆記裡沒人接**
是這個專案最常犯的錯，所以用一支測試雙向守著。**現在表上一份「未接」都沒有。**

**沒有一項擋著把遊戲玩完。** 畫面上該有的框、標籤與計量表都齊了，
蓋氏計數器逼近輻射區時也會滴答，中文行距定案維持 0（24 點漢字剛好填滿字元格）。
剩下的一項是拿眼睛看的驗收：開視窗實際玩一輪。

### 完成度自評

以「與 1988 年原版逐機制對照」為基準的誠實盤點。三個等級：
**照原版** ＝ 判定與資料流照逆向讀出來的實作；**重製呈現** ＝ 判定照原版、
畫面與操作是重製版自己的形狀（記在[接線表](docs/re/00-wiring-status.md)的
該列）；**未做** ＝ 沒有實作，理由列在旁邊。

| 範圍 | 等級 | 說明 |
|---|---|---|
| 主線通關 | 照原版 | 出廠存檔 → 科奇斯基地自毀 → 結局，`TestCochiseEndgame` 全程走過；**人工全程通關還沒有人做過**，這是目前唯一掛著的驗收 |
| 戰鬥（命中、傷害、護甲、行動序、遭遇生成） | 照原版 | 公式與資料表全部從執行檔讀出；戰鬥畫面版面照原版四塊。敵人在地圖上移動已接，但**一整筆遭遇共用一個移動計畫**（原版每組每隻一格，`docs/re/116` 的接線列記著這兩處簡化）|
| 世界互動（移動、時鐘、條件閘、44 個腳本 opcode、問答、寶箱） | 照原版 | 條件閘含通過後改寫；寶箱含現金特例與 lazy 生成 |
| 角色（建立重擲、升級、技能、裝備、裝填、卸卡彈、交給隊友、急救、物品重排） | 照原版 | 含 `+0x25`（護甲索引）、`+0x29`（NPC 旗標）、`+0x2E`（交易計數）三個晚期才解出的欄位；自建 PC 上限 4 照原版 |
| 設施（商店、醫生、訓練師、Ranger Center） | 照原版 | 價格公式、四份庫存表、療傷收費都對過 |
| 存檔 | 照原版 | byte-for-byte round-trip，未解區域一個 byte 不動；原版讀得回去（實機驗過） |
| 中文化 | 完成 | 5,250／5,250 可翻條目 ＋ 段落書 162 段；密語照原版不翻 |
| 選單與清單的呈現 | 重製呈現 | `USE`／撿拾／角色畫面走訊息視窗＋數字鍵，原版是清單框架＋上下鍵；判定不受影響 |
| 名單模式的版面 | 重製呈現 | 中文表頭畫不出原版雙倍高橫幅，改單倍；欄位座標照原版 |
| 三種美術與雙配樂 | 重製版加的 | 原版沒有的東西，不作為 parity 證據 |
| 物品重排 | 照原版 | 角色畫面物品頁按 `R`（原版 Ctrl+R），照想要的順序點完就是新順序，裝備索引跟著搬（`docs/re/134`）；技能清單的重排沒做 |
| 按鍵巨集（F1–F10 錄放） | 未做 | 功能鍵讓給重製版的說明／設定／快速存讀檔；巨集資料照樣 round-trip |
| 複製保護 | 不移植 | 設計決策：段落書直接做進手札 |
| `USE` 的 Medic／Doctor 急救 | 照原版 | 對 CON < 0 的隊員施用，成功把負血折半（`docs/re/133`）；目標選擇改成明確選人（原版把方向鍵硬當記錄編號是已確認的粗糙處）。戰鬥中的 Medic 未接 |
| `TRANSTBL`／資源 idx 7 | 無消費端 | 三層掃描找不到讀取端，原版遺留（`docs/re/56`） |

一句話總結：**規則層與資料層是照原版的，呈現層有列得出來的重製決策，
掛著的是人工通關驗收與兩條不影響判定的原版小路。**
**44 個腳本 opcode 全部實作完**，包含那些要靠改寫存檔才走得到的。
說明書、手冊、段落書與兩份雜誌攻略都已完成，**兩份的機制斷言都逐條拿逆向驗過**：
[軟體世界 1990](docs/walkthrough/swm-005/mechanics-claims.md) 31 條（相符 26、
有出入 5、**未定 0**）、[精訊 1988](docs/walkthrough/jingxun-027/mechanics-claims.md)
14 條（相符 9、版本差異 2、有出入 1、未定 2）。前者最後一條是靠傳送表收掉的：
攻略說的「廢坑」與「彈藥庫」是同一張圖（資源 31），寶箱裡就躺著兩支 M1989。
...
五份文本整理成 markdown：軟體世界 1990 年中文說明書（33 張掃描逐頁轉錄，
**保留當年的譯名與行文**，不用現代用語潤飾）、官方英文手冊（中英對照）、段落書、
**兩份當年的雜誌攻略**（[精訊 1988](docs/walkthrough/jingxun-027/) 與
[軟體世界 1990](docs/walkthrough/swm-005/)，各自附一份「機制斷言逐條拿逆向驗過」的
對照表），以及一份**自建的攻略**。
