...
| **地圖**：底部是原版的指令列，熱鍵字母照原版不跟著翻譯走 | **訊息**：4,912 條可翻字串已全部翻完 |
...
| 中文化 | 可翻字串 4,912／4,912（原版語料 4,806 ＋ 重製版介面 106）、段落書 162／162 |
...
RE 筆記 107 份，每一份都在[接線表](docs/re/00-wiring-status.md)登記
「這個結論 remake 用上了沒有」——**編得過、測得過、玩得動的結論躺在筆記裡沒人接**
是這個專案最常犯的錯，所以用一支測試雙向守著。

剩下的工作分成三類，**沒有一項擋著把遊戲玩完**：原版就沒有消費端的
（`CURS`、`TRANSTBL`）、還沒對拍的一塊畫面（戰鬥的肖像框）、
驗收（設施的實機對拍、開視窗的人工試玩）。**44 個腳本 opcode 全部實作完**，
包含那些要靠改寫存檔才走得到的。說明書、手冊、段落書與攻略四份文件都已完成。
